...
5. **Check for dead code**, in a tree with no compiled output:
   ...
   npm run knip
   ```
   `--clean` removes what `tsc` emitted; the few non-TypeScript assets `npm run build` copies into `packages/tool-server/dist` are left behind. Knip does not look at them, so the count is the same either way.
