...
| `argent map`       | Crawl an app into a graph of its reachable screens and render it live in the preview window's Map tab (behind the `argent-map` flag — run `argent enable argent-map` first)                               |
