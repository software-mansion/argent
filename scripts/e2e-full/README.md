...
| `ios-duo`         | a booted iPhone Duo (`E2E_IOS_DUO_UDID`) with DuoProbe installed, macOS, opt-in | the foldable tier: `fold` through every posture, screenshots and taps on the live panel (the probe app confirms where each tap landed), a recording across a fold, a fold inside `run-sequence`, and the refusal on a non-foldable simulator (`E2E_IOS_FLAT_UDID`)                             |
