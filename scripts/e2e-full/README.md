...
| `device-provider` | an Android device (or `E2E_PROVIDER_IOS_UDID`)                                  | the external device provider contract: the harness spawns argent's own `argent-simulator-server`, publishes a descriptor naming it, and drives the resulting `ext:` device — then asserts argent never killed it, refused `boot-device`, honoured capability denial, and reacted to revocation                                                                                                                                                                                                                              |
| `ios-duo`         | a booted iPhone Duo (`E2E_IOS_DUO_UDID`) with DuoProbe installed, macOS, opt-in | the foldable tier: `fold` through every posture, a sweep between two mid angles and the transient hand-over at 78°, screenshots and taps on the live panel (the probe app counts each tap and confirms where it landed), a flow `swipe: down` unfolded and closed, a flow tap, a bare tap and a screenshot right after a fold made outside argent with no describe in between (`E2E_DUO_HINGE`), a recording across a fold, a fold inside `run-sequence`, and the refusal on a non-foldable simulator (`E2E_IOS_FLAT_UDID`) |
...
intentionally out of scope, except the opt-in `ios-duo` tier, which runs only when
named (`--phase ios-duo`) and given a booted iPhone Duo.
