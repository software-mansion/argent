...
| `ios-duo`         | a booted iPhone Duo with DuoProbe (`E2E_IOS_DUO_UDID`), macOS | `fold` in every posture and between two mid angles, taps and screenshots on the active panel, flow swipes open and closed, a fold made outside argent (`E2E_DUO_HINGE`), a recording across a fold, `fold` in `run-sequence`, and the refusal on a flat simulator (`E2E_IOS_FLAT_UDID`)        |
...
intentionally out of scope. The one exception is the opt-in `ios-duo` tier. It runs
only when you name it (`--phase ios-duo`) and give it a booted iPhone Duo.
