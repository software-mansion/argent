...
trainer = LigerCETrainer(model=model, processing_class=TKZ, train_dataset=train_ds,
    args=SFTConfig(dataset_text_field="text", max_length=MAXLEN, per_device_train_batch_size=1,
        gradient_accumulation_steps=4, max_steps=150, warmup_steps=10, learning_rate=2e-4,
        logging_steps=5, save_steps=10_000, optim="adamw_8bit", gradient_checkpointing=False,
        fp16=True, bf16=False, lr_scheduler_type="cosine", report_to="none", output_dir="/kaggle/working/out"))
trainer.train()
model.save_pretrained("/kaggle/working/adapter")
tok.save_pretrained("/kaggle/working/adapter")
open("/kaggle/working/TRAIN_OK.txt", "w").write("ok")
