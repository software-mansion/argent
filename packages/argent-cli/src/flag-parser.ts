...
/**
 * A retired key: `z.never()` serializes to `{"not": {}}` with no `type`. Matched
 * so usage can render it as a retirement notice rather than offer it as a flag.
 * The serialized shape is pinned in
 * packages/registry/tests/zod-to-json-schema.test.ts.
 *
 * Exported for `findMissingRequired`, which must read the same filter usage
 * renders through.
 */
...
/**
 * A retired field's guidance, minus the leading "Retired: " label — every caller
 * already says retired. Shared by the usage notice and the parse error so the
 * two cannot disagree.
 */
 ...
 * cannot be passed in any form, so every spelling resolves to the same refusal
 * rather than each finding its own (wrong) branch below.
 */
function retiredFieldFor(flag: string, properties: Record<string, JsonSchema>): string | undefined {
  ...
  // The first candidate that is a declared property wins, and it is refused only
  // if that property is retired. Trying the whole token first keeps a field
  // literally named `no-x` readable as itself rather than as `x`'s negation.
...
export function parseFlags(argv: string[], schema: JsonSchema | undefined): FlagParseResult {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    // below and would land in the unknown-scalar tail, where bare `--name` takes
    // the next flag as its value. Refusing before the lookahead makes the rename
    // guidance reachable from every spelling that used to work.
...
export function formatSchemaUsage(schema: JsonSchema | undefined): string {
  ...
  // Retired keys get a notice, not a flag row, which would offer them in the
