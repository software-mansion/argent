...
`$ARGENT_OUTPUT` names the output document of the step, as a file. Argent puts the document the flow holds into that file before the step, and reads the file back after exit code 0. The content must be a JSON object, in UTF-8, of at most 1 MiB. Write a sibling file and `mv` it into place:
...
- **Use LF line endings.** The usual CRLF symptom is `$'\r': command not found` and exit code 127. Add `*.sh text eol=lf` to `.gitattributes`
- **Commands resolve against the PATH of the tool-server**, not against the PATH of your shell. A command that is absent from that PATH exits 127. Restart the tool-server after you change the PATH. On Windows, the bash of Git for Windows puts its own directories first, so `curl` and `sed` resolve to its copies
