...
says what Argent measured and what it changed. Argent does not press the Enter for a
failed step. Before it fails a step, Argent makes one repair where it can prove which
characters are its own: it deletes those characters, then types the text again in small
pieces. A step also fails when that repair ran and Argent could not read the field again
to confirm it.
