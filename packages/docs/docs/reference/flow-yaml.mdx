...
- **A background job ends with the step.** Argent stops the process group of the step, which holds bash and every job bash started. A job that leaves that group survives: `set -m` gives each job a group of its own, and `setsid` does the same. Argent never stops such a job, so stop it yourself. `setsid` is absent on macOS
