...
| `env`     | no       | Environment values for this script. Put `env` inside the `script` map. See [Environment values](#environment-values) |
...
Argent removes each other name. Argent removes its own token, its own port and each `ARGENT_SECRET_` value. Argent does not read a project `.env` file, so a name such as `NODE_ENV` or `DATABASE_URL` is absent by default. Use an `env` map for additional values. To inherit a tool-server variable, add its name to `scripts.env.allow`. See [Environment values](#environment-values).
...
Argent hides resolved secrets in script logs. See [Secrets in an `env` value](#secrets-in-an-env-value).
...
Use `env` to supply script inputs. An `.mjs` script reads `process.env.NAME`; a `.sh` script reads `$NAME`.

Put defaults in the flow file's top-level `env`. Put values for one script inside its `script` map:
...
To change a default for one run, use `--env`:

```bash
argent flow run checkout --env API_URL=https://staging.example.com
```

Argent applies values in this order. Each source replaces matching names from earlier sources:

1. The tool-server's allowed environment variables, including names in `scripts.env.allow`.
2. The root flow's top-level `env`.
3. Each nested flow's `env`, from the outermost flow to the innermost flow.
4. The run's `--env` values or the `flow-execute` `env` parameter.
5. The script step's `env`.

Thus, `--env` replaces flow defaults, but a script step's `env` takes priority over `--env`.

A `run:` step inherits environment values. Its flow's defaults apply only inside that flow. A `tool: flow-execute` step starts a separate run. Pass its required values in `args.env`.

Use a plain YAML map with string values. Quote numbers and boolean values. Variable names must match `[A-Za-z_][A-Za-z0-9_]*`. On Windows, do not use two names that differ only in letter case.

Argent rejects these reserved names:

- `NODE_OPTIONS`, `NODE_CHANNEL_FD`, `NODE_UNIQUE_ID`, `NODE_CHANNEL_SERIALIZATION_MODE`
- `ELECTRON_RUN_AS_NODE`, `ARGENT_FLOW_SCRIPT_RUNNER`, `ARGENT_OUTPUT`, `__proto__`
- `npm_config_node-options`, `npm_config_userconfig`, `npm_config_globalconfig`, including alternative npm spellings

Values cannot contain NUL characters or unpaired surrogates. An `env` value cannot reference a script's output.

Recording tools do not create the top-level `env`. Add it to the YAML after recording. Before you record again, save any defaults: `flow-start-recording` clears the whole file.

### Secrets in an `env` value

Use `{{secret:NAME}}` for a credential. Keep `secret` lowercase and omit spaces inside the placeholder. Argent reads the value on the tool-server's machine from the sources in [Secrets](/docs/reference/configuration#secrets).

A missing secret fails the script step before the script runs. A missing secret in `--env` or the `flow-execute` `env` parameter prevents the run from starting.

Argent hides resolved secrets in script failure messages, stack traces and logs. Redaction does not cover every encoding of a secret. Do not print credentials. Other `env` values remain visible in flow files, reports and tool logs.

Argent does not redact the output document that a script returns. Do not put credentials or other sensitive values in that document.

### The environment of the tool-server

The tool-server keeps the environment from its start, including `PATH`. A later shell `export` does not change it. `scripts.env.allow` adds variable names to the allowlist; it does not update their values.

If a script reports `command not found`, supply the required `PATH` through `env` or restart the tool-server with that `PATH`. Changes to secrets files apply without a restart.
