...
The extension selects the interpreter. An `.mjs` file runs under Node.js, and a `.sh` file runs under bash. There is no `language` key. Argent reads the extension of the file the path resolves to, so a symlink takes the interpreter of its target. A target that has no extension keeps the extension you wrote in the step. See [Bash scripts](#bash-scripts).
