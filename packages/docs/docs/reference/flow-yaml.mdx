...
A `debugger-connect` or `debugger-log-registry` step that succeeds and returns a `note` passes with a warning. The warning repeats the note. The note tells you what became of a session that ended while it held console logs. It gives the log file when that session kept one. It can also say that no file was kept, or that a kept one was removed. Both tools remove that record when they report it, so no later call repeats it. A `debugger-log-registry` note can also say that the current session's log file is not on disk. Nothing removes that one. The writer opens its file one time only, so the note repeats on every call for the rest of the session. Usually `~/.argent/tmp` is not writable. Correct that, then connect the debugger again to get a session with a log file. Other tools that return a `note` do not raise a warning.
