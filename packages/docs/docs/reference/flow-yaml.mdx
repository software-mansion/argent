...
A `script` step runs a local script file in a new process. Use a script for work that no device step can do, for example to seed a database or to clean up after a run.

The extension selects the interpreter. An `.mjs` file runs under Node.js, and a `.sh` file runs under bash. There is no `language` key. See [Bash scripts](#bash-scripts).
...
- The identity, shell, locale, terminal, temporary-directory, cache and configuration names of the host
...
Argent removes every other name. Argent removes its own token, its own port and each `ARGENT_SECRET_` value. A name such as `NODE_ENV` or `DATABASE_URL` is absent, even when your shell sets it. Argent does not read a project `.env` file. There is no `env` key. Let the script read a secret or a URL from a file.
...
A script returns a document through the `output` global. Assign the value to `globalThis.output`, or set a key on the `output` object that Argent puts there. Argent ignores an `export default` value: the step passes with an empty document. The document must be a plain object of JSON-compatible data. `flow-add-script` returns the document as `outputJson`. No flow step reads it yet.
...
| ------- | ---------- | -------------------------------------------------------------------------------------------------------------------------- |
| Failed  | The script | A missing file, a load error, a thrown error, a non-zero exit code, or an `output` value that Argent cannot serialize      |
...
The exit code is the verdict. Exit code 0 passes the step, and a different code fails it. Exit code 126 means that bash cannot read the file, or that a command in the file is not executable. Exit code 127 means that bash did not find a command.
