...
Apple TV and Android TV are not flow targets. Their ids classify as `ios` and `android`, so the runner accepts the touch directives and dispatches them at a focus-driven UI where they do nothing useful — nothing in the engine rejects them, and an author must not write them. `keyboard` still refuses `key` and `clear` on both. Drive those two with the tools directly (`describe` for focus, `tv-remote`, `keyboard` with `text`).
