...
A background job can hold stderr after bash exits. A line that such a job writes when Argent stops it goes only to the log. A job can also still write to stderr when Argent stops waiting. Then the second part is the last non-blank line that the script wrote to stderr before bash exited.
