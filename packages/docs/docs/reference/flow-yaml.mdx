...
A flow that fails its steps lets the directory run continue. So does a flow that the tool-server rejects up front, such as an invalid file or a device that Argent cannot resolve. A transport failure, a rejection of another kind, or a reply that is not a report ends the directory run. Argent counts the flows after it as skipped.
...
`--json` and `--json-stream` print no header, no outcome line and no summary. A run in these modes writes only JSON to stdout.

A directory run with `--json` writes one document. The document holds `ok`, `total`, `passed`, `failed`, `skipped` and a `flows` array. The array holds one entry for each flow. Each entry holds the `path` and the `status`. An entry holds `report` when the flow produced one. An entry holds `error` when Argent did not get a report from the tool-server. Such an entry also holds `error_code` and `error_kind` when the tool-server sends them. `--json-stream` puts the same two fields on its `error` record.

A single flow run with `--json` writes the report of that flow. When such a run produces no report, Argent writes nothing to stdout and puts the reason on stderr. Use `--json-stream` to get the failure as a record.
