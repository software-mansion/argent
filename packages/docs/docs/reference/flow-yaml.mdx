...
`within` uses the visual frame, not the source tree. Reading order is top to bottom, left to right, on the screen as the user sees it: on an iOS simulator whose UI is landscape, a rotated device or an unfolded foldable, Argent reads it in the UI's orientation while the frames stay in the screen's portrait space. Scopes nest, with at most six scope keys in one selector. The `await-ui-element` tool does not support scopes.
...
| `fold`       | `<posture>`, `<angle>` or `{ posture }` / `{ angle }`              | Fold or unfold a foldable iOS simulator and wait until the panel it renders to takes input. `posture` is `closed`, `half-open` or `open`; `angle` is 0 to 180 degrees; one of the two. The sweep starts on the panel the device renders to, so a fold made outside the flow needs nothing extra |
...
A direction (`swipe: down`, `scroll-to` with `direction`) is the direction on the screen as the user sees it. On an iOS simulator whose UI is landscape, a rotated device or an unfolded foldable, Argent turns it into the space the frames and touches are in, which stays the screen's portrait space. Reading order, which `after`, `next`, `any: true` and a `text` condition go by, is the user's in the same way. Coordinates (`tap: { x, y }`, `swipe` with `from`, `to` or `by`) are in that frame space and are not turned.
...
A swipe moves one finger across the screen. `direction` is the travel of the finger, not the travel of the content, on the screen as the user sees it: on a landscape UI Argent turns it into the frame space (see above). To bring an element on screen use `scroll-to` not `swipe`.
