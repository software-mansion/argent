...
`$ARGENT_OUTPUT` gives the path to a file that initially contains `{}`. To return data, write a UTF-8 JSON object to this file. The size limit is 1 MiB. Write to a temporary file first, as shown below:
...
If the script returns no data, leave `$ARGENT_OUTPUT` unchanged. Invalid output fails the step even with exit code 0. An empty or missing output file also fails the step.
