...
`ARGENT_OUTPUT` and `ARGENT_REASON` are refused whichever language the step runs, because a flow-level map applies to every step. `__proto__` is refused too. It matches the name rule, but every map on the way to the child copies it through a plain object, where the name is an accessor rather than an entry, so the value would be dropped and the step would pass anyway. That refusal covers the two YAML maps and `--env`. On the two tool parameters, `flow-execute`'s `env` and `flow-add-script`'s, the schema drops the name before any rule of Argent runs, so the call passes with the value missing and no refusal.
...
The remedies are to restart the tool-server, to name the value in `scripts.env.allow`, or to pass an absolute path through an `env` value. A failure that reads as a missing command carries a note that says so. `scripts.env.allow` does not fix a missing command: that key adds names to copy out of the snapshot, and `PATH` is copied already.
