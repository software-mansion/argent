...
A background job can hold stderr after bash exits. Argent counts the stderr lines that follow the exit of bash until stderr is quiet for 500 milliseconds. A line that arrives after that goes only to the log. If stderr is never quiet that long, the second part is the last non-blank line that the script wrote to stderr before bash exited.
