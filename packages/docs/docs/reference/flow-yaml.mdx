...
`type` always appends: it focuses the element and types, and it has no clear form. Into a field that already holds a value — one the app remembered, or one an earlier run of the same flow left — that splices the new characters into the old one, and the flow replays that wrong value on every run. Where the field can arrive non-empty, write the steps by hand instead:
