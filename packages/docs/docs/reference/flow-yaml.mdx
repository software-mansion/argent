...
| `fold`       | `<posture>`, `<angle>` or `{ posture, angle, from }`               | Fold or unfold a foldable iOS simulator. `posture` is `closed`, `half-open` or `open`; `angle` is 0 to 180 degrees; one of the two. `from` is where the hinge is now |
