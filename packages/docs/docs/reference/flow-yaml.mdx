...
gives the counts and the probable cause. Argent does not press the Enter for a failed
step. Argent makes one repair before it fails a step: it deletes the characters that it
can prove are its own, then types the text again in small pieces.
