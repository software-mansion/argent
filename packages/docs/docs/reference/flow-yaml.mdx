...
| `type`       | `{ into: <selector>, text, submit }`                               | Focus the element and type. Presses Enter unless `submit: false`. See [Typed text](#typed-text)   |
...
On an Android phone or tablet, the tool-server reads the field back after a `type` step.
The step fails when the read-back proves that the text is not in the field. The reason
says what Argent measured and what it changed. Argent does not press Enter for a failed
step. Argent makes one repair when the read-back proves the text did not land. The repair covers only
the characters that Argent can prove are its own. It deletes those characters. It then
types the text again in small pieces. A step passes when the repair works. The app then saw the text
typed twice, and a repair that deleted first also changed the field by more than the typed
text. A step also fails when the repair ran and Argent could
not read the field again to confirm it. A step fails as well when the repair failed partway,
because the repair deletes before it types.

A raw `tool: keyboard` step fails on the same verdict, and so does a `tool: run-sequence`
step that holds one. The flow recorder writes a typed step in the raw form, so a recorded
flow carries the same gate.

The step passes when the read-back cannot conclude, and on every other platform. A step
that passes is therefore not a proof that the text arrived. The step report carries the
read-back note as a warning, so an unconfirmed pass stays visible. See the [tools
reference](/docs/reference/tools#typed-text-on-android).
