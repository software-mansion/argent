...
step. Argent makes one repair before it fails a step. The repair covers only the characters
that Argent can prove are its own. It deletes those characters. It then types the text
again in small pieces. A step also fails when the repair ran and Argent could not read the
field again to confirm it.
