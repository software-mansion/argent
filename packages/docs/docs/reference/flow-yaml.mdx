...
A run on a remote simulator uses the `ios` entry.
...
Baselines live in `.argent/flows/__baselines__/<flow>/`. Argent keys a baseline by platform and capture geometry, plus the selector for `cropOn`. A remote iOS simulator uses the iOS key, so a cloud run compares against the baseline a local run of the same device model committed. Run `argent flow run <name> --update-baselines` to write the baselines from a known-good state. Review each baseline before you commit it.
