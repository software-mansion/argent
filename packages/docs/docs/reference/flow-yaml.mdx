...
The reason of a step that did not pass states what failed. Under that step, Argent prints each of these lines that applies to the failure. The CLI and the result of the `flow-execute` tool show the same lines:

| Line             | Content                                                                                                                              |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `expected:`      | The value that the check wanted: a text, a snapshot tolerance or an image size                                                       |
| `actual:`        | The value that the check found. Argent shows the first 300 characters of a text, then the number of characters that it does not show |
| `indeterminate:` | The check did not run, because Argent could not read the UI tree                                                                     |
| `hint:`          | A thing to try first, or a fact that helps you find the cause                                                                        |
...
Argent prints each value on one line. An invisible character in a value becomes an escape, for example `\n` for a line break or `\u00a0` for a no-break space. These characters are invisible:

- Control characters, for example a line break or a tab.
- Format characters, for example a zero-width space.
- Each space character other than the usual space.

Thus a value that differs only by an invisible character looks different from the other value.

Argent prints a text value in quotation marks, with the escapes of JSON: `\"` for a quotation mark and `\\` for a backslash. A `hint:` line shows a text from the device in the same format. Argent prints a regular expression between slashes, as on the step line. The text between the slashes is the value of `matches:`.

```
  ✗  4 assert id=count matches /^Taps: \d\d\d$/ (1.0s) — element matched id="count" but its text did not match /^Taps: \d\d\d$/
       expected: /^Taps: \d\d\d$/
       actual:   "Taps: 0"
```
...
- The `expected:`, `actual:`, `indeterminate:` and `hint:` lines of that step that apply to the failure.
...
When a single flow run fails its steps, Argent prints the first step that failed or errored again above the last line. Under that step, Argent prints its reason and its `expected:`, `actual:`, `indeterminate:` and `hint:` lines again.
...
A single flow run with `--json` writes the report of that flow. The report holds `startedAt`, the start of the run in epoch milliseconds, and `durationMs`, the time of the run in milliseconds. Each step that ran holds its own `durationMs`. A step that did not pass can also hold `hint`, `expected` and `actual`, with the content of the lines above. `actual` holds all of the text that the check found. A step whose `expected` holds a regular expression also holds `expectedKind: "pattern"`. The `expected` field of such a step holds the source of the pattern, without the slashes. A step holds `indeterminate: true`, shown as the `indeterminate:` line, when the step did not do its check because Argent could not read the UI tree. A `tool: flow-execute` step copies these fields from the first step that failed in the composed flow. Each `progress` record of `--json-stream` holds one step, with the same `durationMs`. When the run gets no report, Argent puts the reason on stderr instead. Use `--json-stream` to get the failure as a record.

These steps can hold `indeterminate: true`:

- `await`, `assert`, `idle` and a `when` guard. Each read of the UI tree failed or was empty, or the reads stopped at the end of the wait.
- A step that must find an element: `tap`, `long-press`, `type`, `swipe`, `pinch`, `rotate`, or a `snapshot` with `cropOn`. Each read of the UI tree was empty, and the tree source marked the read as bad. This occurs only on Vega, when the automation toolkit is not attached. When the tree source fails with an error, such a step gets the status `error` and no flag.

For a step with the flag, the app can be the cause: a crash, or a screen that the app made empty. The device or the tree source can also be the cause. Look at the app first. Then run the flow again before you change it.

A step gets no flag when Argent refuses to read the UI tree, for example for an Apple system app. A new run gets the same refusal. Change the flow as the reason tells you.
