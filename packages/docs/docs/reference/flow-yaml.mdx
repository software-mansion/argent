...
Add top-level `env` after recording. Save it before re-recording: `flow-start-recording` clears the whole file.
...
If no alternative supplies a value, the step stops the run with an error. This also applies to `when` conditions. References support only paths and `??` fallbacks. Calculate other values in the script.
