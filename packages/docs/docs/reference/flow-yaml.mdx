...
`type` always appends: it focuses the element and types, and it has no clear form. A field can already hold a value: one the app remembered, or one an earlier run of the same flow left. `type` then splices the new characters into the old value, and the flow replays that wrong value on every run. Where the field can arrive non-empty, write the steps by hand instead:
...
The last step replaces the Enter that `type` sends. `type` presses Enter unless it carries
`submit: false`. A `tool: keyboard` step never does, so a replacement without that step fills the
field and never submits it. Drop the step where the original `type` carried `submit: false`.

The `wait:` is load-bearing: raw `tool:` steps take no settle. On every target except Chromium a clear that arrives before focus lands empties the previously focused element and still reports success. On Chromium the clear fails when nothing editable holds focus, but it cannot tell one editable element from another. On iOS, Android and the TV targets the burst clears up to 100 characters on each side of the cursor, so a longer field needs a second `clear` step; on Vega it clears up to 200 characters before the cursor and none after it; on Chromium there is no limit and the result carries `clearVerified` when the field was read back empty.
...
| Vega         | Use `tool: tv-remote` and `tool: keyboard`. `keyboard` takes `text`, `key` or `clear`. The runner rejects the touch directives (`tap`, `long-press`, `swipe`, `type`, `scroll-to`, `pinch`, `rotate`) before it resolves a selector |

Apple TV and Android TV are not flow targets. Their ids classify as `ios` and `android`, so the runner accepts the touch directives instead of rejecting them, and an author must not write them. On Android TV they reach the device and do nothing useful at a focus-driven UI. On Apple TV they fail: every touch directive resolves the simulator-server, which refuses a tvOS id outright, so the step errors and the flow stops. `keyboard` still refuses `key` on both. Drive those two with the tools directly (`describe` for focus, `tv-remote`, `keyboard` with `text` or `clear`).
