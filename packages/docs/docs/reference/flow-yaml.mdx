...
A direction (`swipe: down`, `scroll-to` with `direction`) is the direction on the screen as the user sees it. On an iOS simulator whose UI is landscape, a rotated device or an unfolded foldable, Argent turns it into the space the frames and touches are in, which stays the screen's portrait space. Coordinates (`tap: { x, y }`, `swipe` with `from`, `to` or `by`) are in that frame space and are not turned.
