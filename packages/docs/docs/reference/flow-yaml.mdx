...
Baselines live in `__baselines__/<flow>/` next to the flow file. Argent keeps the baselines of a nested flow in `.argent/flows/<subdir>/__baselines__/<flow>/`. Argent keys a baseline by platform and capture geometry, plus the selector for `cropOn`. Run `argent flow run <name> --update-baselines` to write the baselines from a known-good state. Review each baseline before you commit it.
...
| `argent flow run`             | Run every flow `argent flow list` prints             |
...
| `--update-baselines` | Write the snapshot baselines instead of comparing them. Argent refuses this option on a bare `argent flow run`, because the target must be one you named                          |
| `--output <dir>`     | Write the failed baseline, current and diff images to `<dir>/<flow>/`, for a CI artifact upload. Argent keys a nested flow as `<dir>/<subdir>/<flow>/` on a directory or bare run |
| `-r, --recursive`    | With a directory path, also run the flows in subdirectories. A bare `argent flow run` runs them without this option                                                               |
...
The file name without `.yaml` names the report and the artifacts. It contains only letters, numbers, `_` and `-`. A directory run prints only the failing steps and a summary. A bare `argent flow run` walks the subdirectories. `--recursive` does the same for a directory path. Both forms skip dot-directories, `node_modules` and `__baselines__`. `argent flow list` skips the same directories. An invalid file fails alone and the batch continues. An infrastructure error stops the batch and counts the remaining flows as skipped.
