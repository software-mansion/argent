...
A command that the script starts can write to the log after the script exits. Argent waits for such a command while it writes, for up to 3 seconds. After that, Argent stops reading the log. Argent then marks the log as cut and adds a note to the step.
...
After the script exits, a step waits while a command that the script started still writes to the log, for up to 3 seconds.
...
A background job can hold stderr after bash exits. A line that such a job writes when Argent stops it goes only to the log. If the job still writes when Argent stops waiting for the log, the second part is the last non-blank line that the script wrote to stderr before bash exited.
