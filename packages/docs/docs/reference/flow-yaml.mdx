...
Write a credential as `{{secret:NAME}}`. Argent resolves it on the machine that runs the flow, from the sources in [Secrets](/docs/reference/configuration#secrets), just before the process starts. A name that no source defines errors the step, and the message lists the names that are available.
