...
- **A background job ends with the step.** A job survives only after `setsid`, and Argent never stops a `setsid` job. Do not reap jobs with `trap 'kill 0' EXIT`: `kill 0` also kills bash, and the step fails
