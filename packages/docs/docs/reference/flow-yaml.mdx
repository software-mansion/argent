...
A `tool` step runs on the tool-server, so an argument that names a path on the agent's own machine
(`screenshot`'s `out`) has no effect in a flow. The step passes with a warning naming the path it
did not write.
