...
All fields in a selector must match. An unqualified Android id also matches its qualified resource id. `identifier` is an alias of `id`. Use single quotes for a regex with a backslash.
...
The `flow-execute` tool takes one flow source: `name` for a saved flow, or `flow_path` for any file. `run:` targets and baselines resolve on the file system of the tool-server. When the tool-server runs on a different machine, a `name` run sends only that one file. Thus a `run:` step fails as a missing fragment and a `snapshot` fails for a missing baseline. `flow_path` is refused when the agent and the tool-server do not share a file system.
