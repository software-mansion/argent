...
| --------------- | ------------------------------------------------------------------ | --------------------------------------------------- |
| End-to-end flow | `launch:`, direct or inside a leading `repeat:` block with a count | Argent starts the app from scratch                  |
| Fragment        | Any other step                                                     | The app is already in the state that the flow needs |

Argent skips leading `echo` steps when it classifies a flow. A flow that opens with an `echo` step and then a `launch:` step is still end-to-end. Argent also looks inside a leading [`repeat:` block](#repeated-steps) that has a count, because the first iteration runs the launch as step 1. Argent does not look inside a leading `repeat:` block that has an `until` bound, because the body can run zero times. Argent classifies that flow as a fragment. Put the named start state of an end-to-end flow in a leading `echo` step.
...
| `repeat`     | `<count>` or `{ until: <condition>, max }`, with `steps:`          | Run the nested steps to a count or until a condition holds. See [Repeated steps](#repeated-steps) |
...
`idle` has no `assert` form, no `when` form and no `until` form.
...
- repeat: { until: { hidden: { text: Clear all } }, max: 15 }
  steps:
    - tap: { text: Clear all }
    - await: { hidden: { text: Clear all } }
```

A `repeat:` step takes exactly one bound and a `steps:` list:

| Bound                            | Behavior                                                                       |
| -------------------------------- | ------------------------------------------------------------------------------ |
| `repeat: <count>`                | Run the steps `<count>` times. `repeat: { times: <count> }` is the same bound  |
| `repeat: { until: <condition> }` | Run the steps until the condition holds. `max` bounds the number of iterations |

`times` and `max` take a literal integer from 1 to 100. The default `max` is 10.

An `until` guard takes one `exists`, `visible`, `hidden` or `text` condition. An `until` guard rejects `platform`, `idle` and `timeout`. Argent checks the guard before each iteration, also before the first. When the guard already holds, Argent runs the steps zero times and passes the step. When the guard does not hold after `max` iterations, Argent fails the step.

A `repeat:` block is not a retry. A failure inside an iteration stops the flow. When a step fails sometimes, add an `await:` step for the state that the step needs.
...
Argent refuses a `snapshot:` step inside a [`repeat:` block](#repeated-steps). A snapshot name maps to one baseline, and each iteration can show a different screen. Argent refuses the step in every `repeat:` block, also in a block with a bound of 1. Put the snapshot after the block.
