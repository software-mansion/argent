...
| `fold`       | `<posture>`, `<angle>` or `{ posture, angle, from }`               | Fold or unfold a foldable iOS simulator and wait until the new panel takes input. `posture` is `closed`, `half-open` or `open`; `angle` is 0 to 180 degrees; one of the two. `from`, optional, is the angle the hinge was left at by something other than the flow |
