...
A command that the script starts can write to the log after the script exits. Argent waits for such a command while it writes, for up to 3 seconds. After that, Argent stops waiting for the command. Argent then marks the log as cut and adds a note to the step.
...
After the script exits, a step waits for up to 3 seconds while a command that the script starts still writes to the log.
...
A background job can hold stderr after bash exits. Argent counts the stderr lines that follow the exit of bash until stderr is quiet for 500 milliseconds. A line that arrives after that goes only to the log. The wait for the log can reach its limit while stderr still runs. Then the second part is the last non-blank line that the script wrote to stderr before bash exited.
...
Argent runs each candidate in the working directory of the step and asks it for its version. A version-manager shim, such as the shim of asdf, thus finds the bash that the project pins. Argent refuses a candidate that does not answer with a bash version. If the candidate exits without a version, the refusal quotes the first non-blank line that the candidate wrote to stderr. When Argent refuses the `bash` on the PATH and takes a later candidate, Argent adds a note to the step.
