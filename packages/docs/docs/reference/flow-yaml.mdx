...
The `wait:` is load-bearing: raw `tool:` steps take no settle. On iOS and Android a clear that arrives before focus lands empties the previously focused element and still reports success. On Chromium the clear fails when nothing editable holds focus, but it cannot tell one editable element from another. On iOS and Android the burst clears up to 100 characters on each side of the cursor, so a longer field needs a second `clear` step; on Chromium there is no limit and the result carries `clearVerified` when the field was read back empty.
...
| Vega         | Use `tool: tv-remote` and `tool: keyboard`. `keyboard` takes `text` or `key`; `clear` is rejected. The runner rejects the touch directives (`tap`, `long-press`, `swipe`, `type`, `scroll-to`, `pinch`, `rotate`) before it resolves a selector |

Apple TV and Android TV are not flow targets. Their ids classify as `ios` and `android`, so the runner accepts the touch directives instead of rejecting them, and an author must not write them. On Android TV they reach the device and do nothing useful at a focus-driven UI. On Apple TV they fail: every touch directive resolves the simulator-server, which refuses a tvOS id outright, so the step errors and the flow stops. `keyboard` still refuses `key` and `clear` on both. Drive those two with the tools directly (`describe` for focus, `tv-remote`, `keyboard` with `text`).
