...
`timeout` bounds the run of the script, not the time of the step. The tool-server runs a limited number of scripts at the same time, and it takes the number from the CPU count of the host. A step waits for a free slot before the script starts. The time that Argent reports for the step includes this wait. The step reports a wait of more than 5 seconds in its reason, and the wait can be longer than the `timeout` of the step. A step that finds the queue full is an errored step. Every project on the host shares one queue, because every project shares one tool-server.
...
Argent adds a time to the output lines in this table. On a step line, the time comes before the reason. On the other lines, the time comes last. Argent rounds a time to 0.1 seconds. A result below one minute shows as `(1.2s)`. Other times show in whole minutes and seconds, as `(1m 32s)`.

| Line                       | Time                                                                                                                      |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| Step line                  | The time of the step. A skipped step shows no time. An `echo` line shows no time                                          |
| `when` step line           | The time of the guard only. The line of an unmet guard also shows the time. An unmet UI guard uses the full 1000 ms grace |
| `run` step line            | The time to load the fragment only                                                                                        |
| Last line of a flow run    | The time of the run. This time includes the read of the flow file, the device resolution and the teardown                 |
| Summary of a directory run | The time of the directory run. The CLI measures this time. It also includes the transport and the artifact export         |

The steps in a `when` block or in a fragment show their own times. The time of the run includes the time of each step. The result of the `flow-execute` tool shows the same times.
...
A single flow run with `--json` writes the report of that flow. The report holds `startedAt`, the start of the run in epoch milliseconds, and `durationMs`, the time of the run in milliseconds. Each step holds its own `durationMs`, except a skipped step. The entry of an unmet `when` guard also holds `durationMs`. Each `progress` record of `--json-stream` holds one step, with the same `durationMs`. When the run gets no report, Argent puts the reason on stderr instead. Use `--json-stream` to get the failure as a record.
