...
| `--json-stream`      | Print one JSON record per line: each step, then the result. One flow only                              |
...
A directory run prints a numbered header for each flow. Under the header it prints the failing steps of the flow and one outcome line. It prints a flow-level summary at the end. A flow that the tool-server rejects before the first step fails alone. The batch continues. Any other failure stops the batch and counts the remaining flows as skipped.

The outcome line of a flow that produced a report is the summary of the run. The outcome line of a flow that produced no report states the reason. A single flow run prints the same reason under the name of the flow.
...
| `not run (batch stopped)`        | An earlier flow stopped the batch. Directory runs only     |

`--json` and `--json-stream` print no outcome lines. JSON is the only output on stdout. In the aggregate of a directory run, each entry holds the path and the status of one flow. An entry holds the report when the flow produced one. A failed entry holds the message in `error`. When the tool-server sends a failure signal, the entry also holds `error_code` and `error_kind`. `--json-stream` puts the same two fields on its `error` record.
