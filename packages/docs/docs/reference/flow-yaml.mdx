...
The reason of a step that did not pass states what failed. Under that step, Argent prints each of these lines that applies to the failure:

| Line        | Content                                                                         |
| ----------- | ------------------------------------------------------------------------------- |
| `expected:` | The value that the check wanted: a text, a snapshot tolerance or an image size  |
| `actual:`   | The value that the check found. Argent shows the first 300 characters of a text |
| `hint:`     | The first thing to try                                                          |
...
Argent prints each value on one line. A control character in a value becomes an escape, for example `\n` for a line break. Thus a value that differs only by a line break or a tab looks different from the other value.

Argent prints a text value in quotation marks. Argent prints a regular expression between slashes, as on the step line. Thus you can copy the pattern back into `matches:`.
...
A single flow run with `--json` writes the report of that flow. The report holds `startedAt`, the start of the run in epoch milliseconds, and `durationMs`, the time of the run in milliseconds. Each step that ran holds its own `durationMs`. A step that did not pass can also hold `hint`, `expected` and `actual`, with the content of the lines above. A step whose `expected` holds a regular expression also holds `expectedKind: "pattern"`. The `expected` field of such a step holds the source of the pattern, without the slashes. A step holds `indeterminate: true` when Argent could not read the UI tree to do the check. Argent did not do the check on such a step. These steps can hold the flag: `await`, `assert`, `idle`, a `when` guard, and a step that must find an element — `tap`, `long-press`, `type`, `swipe`, `pinch`, `rotate`, or a `snapshot` with `cropOn`. The app can be the cause: a crash, or a screen that the app made empty. The device or the tree source can also be the cause. Look at the app first. Then run the flow again before you change it. A `tool: flow-execute` step copies these fields from the first step that failed in the composed flow. Each `progress` record of `--json-stream` holds one step, with the same `durationMs`. When the run gets no report, Argent puts the reason on stderr instead. Use `--json-stream` to get the failure as a record.
