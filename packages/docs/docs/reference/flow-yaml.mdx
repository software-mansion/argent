...
On a platform with no UI-tree source for flows (HarmonyOS, a remote iOS device), `idle` cannot run. The step errors immediately and names `wait:` as the replacement.
