...
A `tool` step that succeeds and returns a `note` passes with a warning that repeats the note. A tool adds a note when its own result is misleading on its own: the debugger tools name the log file that a crashed session kept, and `open-url` says that the link opened the browser and not the app. The debugger tools remove the record when they report it, so no later call repeats it.
