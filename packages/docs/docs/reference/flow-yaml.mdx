...
Argent runs the file as `bash <file>`. The file needs no execute bit, and Argent ignores the `#!` line. The script gets no arguments, and its standard input is empty. Argent does not report what the script prints.
...
`$ARGENT_REASON` names an empty file for the failure text. To explain a failure, write the explanation to that file and exit with a non-zero code. Argent reads about 7000 characters of the file and puts them in the reason of the step. A script that writes nothing there reports only its exit code.
...
- **Use LF line endings.** Argent discards what the script prints, so the `$'\r': command not found` line bash writes never reaches you. What you see instead is a step that failed on an exit code the script did not choose, or a step that failed because the redirection landed one carriage return past the name `$ARGENT_OUTPUT` holds. Add `*.sh text eol=lf` to `.gitattributes`
