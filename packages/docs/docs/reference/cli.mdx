...
| `argent lens`      | Open Argent Lens bound to a fresh coding-agent session. Claude by default, `--agent` selects codex, gemini, opencode or cursor (macOS; behind the `argent-lens` flag, run `argent enable argent-lens` first) |
