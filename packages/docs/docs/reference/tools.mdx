...
verdict can follow a repair. The `note` field then says what Argent deleted and typed
again. A long text spends tens of seconds in the repair. Argent leaves the field alone
when it cannot prove which characters are its own.
...
conclude. On Android the `note` field gives the reason. The other platforms send no note,
because they run no read-back. The repair can run before an inconclusive
