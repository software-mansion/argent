...
| `button`                     | Press a hardware button: home, back, power, volume, app switch, action button                 |
...
| `false` | Argent read the field back, and the last measurement is a failure |
| absent  | Argent did not check. This is not evidence either way             |

Argent makes one repair when the read-back proves the text did not land. The repair covers only
the characters that Argent can prove are its own. It deletes those characters. It then
types the text again in small pieces. A call that repairs can thus change the field by
more than the text that it adds, and the app sees both rounds of key events. A `true`
verdict can follow a repair. The `note` field then gives the count of characters Argent
deleted and retyped, never the field's text. A long text spends tens of seconds in the
repair. When Argent cannot prove which characters are its own it leaves the field alone
and reports `false`, with a `note` that gives the character counts of the mismatch.
...
Argent reports `false` for a repair that failed partway too. The field can then hold less
than it held before the call, because the repair deletes before it types. The `note` field
says so.

The repair is not atomic on the device: Argent reads the field, then deletes and retypes in
separate calls, with nothing locking the device between them. A second `keyboard` call on
the same field in that window can skew the counts the deletion is based on, so drive one
controller per device. An interleaved write usually leaves the read ambiguous, which
cancels the repair rather than deleting the wrong characters.
...
`verified` is absent on every other platform, on Android TV — which shares the phone
transport but is never read back — and on an Android phone or tablet when the check cannot
conclude. In that last case the `note` field gives the reason. The other platforms and
Android TV send no note, because they run no read-back. The repair can run before an inconclusive
...
a flow `tool: keyboard` step, and a flow `tool: run-sequence` step that holds one. An absent `verified` stops nothing. A flow step that
passes with a `note` carries that note as the step warning. This covers a `type` step, a
raw `tool: keyboard` step, and a `run-sequence` that holds one.

A cancelled call stops the typing. Argent types nothing when it sees the cancel before the
first keystroke. The iOS and Chromium backends stop between keys. An Android call
stops when the burst it is in has gone out. On the path that verifies, Argent sees a mid-typing
cancel after the read-back that follows the burst, and a cancel inside a repair at the next chunk
or batch of backspaces. A call that skips the read-back sees it right after the burst. A TV target and
Vega type the whole string regardless. A cancel inside an Android repair can leave the
field with fewer characters than it held before the call, because the repair deletes before
it types.
