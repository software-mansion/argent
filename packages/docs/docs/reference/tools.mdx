...
Three groups keep an explicit target:
...
- A flow step and a `run-sequence` step carry no device of their own. Every step runs on the target its parent resolved. A flow pins that target when the flow runs. A sequence takes it from its own `udid`, which resolves like any other tool's.
