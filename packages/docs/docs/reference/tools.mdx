...
and a flow `tool: keyboard` step. An absent `verified` stops nothing. A flow step that
...
stops when the burst it is in has gone out. Mid-typing that is after the read-back that
follows the burst. Inside a repair it is the next chunk or batch of backspaces. A TV target
and Vega type the whole string regardless. A cancel inside an Android repair can leave the
