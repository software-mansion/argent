...
Argent makes one repair before it reports `false`. The repair covers only the characters
that Argent can prove are its own. It deletes those characters. It then types the text
again in small pieces. A `keyboard` call thus changes the field by more than the text that
it adds. A long text spends tens of seconds in the repair. Argent leaves the field alone
