...
| `false` | Argent read the field back and the text is not in it |
...
change the field by more than the text it adds. Argent leaves the field alone when it
cannot prove which characters are its own.
