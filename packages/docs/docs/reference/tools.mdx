...
| `false` | Argent read the field back and did not find the text in it |
| absent  | Argent did not check. This is not a pass                   |

Before Argent reports `false`, it makes one repair: it deletes the characters it can prove
are its own, then types the text again in small pieces. A `keyboard` call can therefore
change the field by more than the text it adds. A long text spends tens of seconds in that
repair. Argent leaves the field alone when it cannot prove which characters are its own.
