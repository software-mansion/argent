...
Argent drives a foldable iOS simulator (the iPhone Duo) in every posture. `list-devices` marks it with `foldable: true`. `fold` moves the hinge to `closed`, `half-open` or `open`, or to an angle from 0 to 180 degrees, and waits until the device renders to the panel for that posture. Closed, the device renders to the cover panel; half-open and open, to the inner panel. Argent names that panel on every screenshot, describe, touch, stream and recording, so the tools follow the fold. The coordinate space changes with the panel: run `describe` again after a fold, and expect a different screenshot size. A fold during a gesture is not supported. Unfolded, `rotate` sets the orientation of the device, not of the UI: `Portrait` gives a landscape UI on the inner panel.
