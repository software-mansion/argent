...
| `restart-app`                | Terminate then relaunch an app (iOS / Android / Vega — not on Chromium)                       |
...
| `debugger-component-tree`      | Fetch the current screen as a compact React component text tree (iOS / Android) |
| `debugger-inspect-element`     | Inspect the React component hierarchy at a screen coordinate (iOS / Android)    |
| `debugger-log-registry`        | Summarize console logs captured from the app                                    |
| `debugger-reload-metro`        | Restart the Metro bundle without restarting the native process (iOS / Android)  |
...
| Tool               | Description                                                                                                                                                                               |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `chromium-tabs`    | List, switch, and close tabs or `BrowserWindow`s, and open a tab on a browser — an Electron app opens its own windows. Needs an existing page, so a closed last window cannot be reopened |
...
All three drive the active page, so an app that is running with no open tab or window fails them before they run. The same holds for `describe`, `screenshot` and every Chromium-capable `debugger-*` tool except `debugger-status` and `debugger-log-registry`, which answer with a `not_connected` result rather than failing. `list-devices` drops such an app exactly as it drops one that exited — so a missing Chromium entry is not evidence the app is gone. Call `debugger-status` on the id you already have: it reports a `reason` and a `guidance` field naming the recovery for the state it found. `restart-app` and `reinstall-app` are refused on Chromium, and `boot-device` only ever starts an app, so quitting one is the user's move.
