...
A cancelled call stops the typing. Argent types nothing when the cancel arrives before the
first keystroke. The iOS simulator and Chromium backends stop between keys. An Android call
stops when the burst it is in has gone out. Argent sees a mid-typing cancel after the
read-back that follows the burst, and a cancel inside a repair at the next chunk or batch of
backspaces. A call that skips the read-back types the string and returns. A TV target and
