...
types the text again in small pieces. A call that repairs thus changes the field by more
than the text that it adds. A `true` verdict can follow a repair. The `note` field then says
what Argent deleted and typed again. A long text spends tens of
...
and a flow `tool: keyboard` step. An absent `verified` stops nothing. A flow `type` step
that passes with a `note` carries that note as the step warning.
...
stops when the burst it is in has gone out. Mid-typing that is after the read-back that follows
the burst. Inside a repair it is the next chunk or batch of backspaces. A cancel inside an
Android repair can leave the field with fewer characters than it held before the call,
because the repair deletes before it types.
