...
The argument is optional. Argent acts on the one booted device that supports the tool when the caller leaves the argument out. A session with a single simulator needs no device id at all. This holds for the agent's tool calls and for the `argent run` command line.

Argent picks a device only when the choice is unambiguous. Each tool declares which platforms it supports. Only booted devices that pass that declaration are candidates. A Chromium tool still resolves when a phone is booted next to the browser, because the phone is not a candidate for it.

The declaration covers platform and device kind, but not form factor. An Apple TV simulator is an Apple simulator like any other. Name the device yourself when a TV and a phone are booted together.
