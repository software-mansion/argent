...
| `fold`                       | Fold or unfold a foldable iOS simulator to a posture or a hinge angle                         |
...
Argent drives a foldable iOS simulator (the iPhone Duo) in every posture. `list-devices` marks it with `foldable: true`. `fold` moves the hinge to `closed`, `half-open` or `open`, or to an angle from 0 to 180 degrees, and waits until the device has settled on the panel it renders to and accepts input there. The sweep starts on the panel the device renders to, so a fold made outside Argent needs nothing extra. Closed, the device renders to the cover panel; half-open and open, to the inner panel. The device switches panels on a sweep from or to closed or open; a sweep between two angles short of those stops can leave it on the panel it had, and the result then reports that panel. Argent resolves the live panel for every screenshot, describe, touch, stream and recording at the moment it runs, so the tools follow the fold, also a fold made outside Argent. The coordinate space changes with the panel: run `describe` again after a fold, and expect a different screenshot size. When Argent cannot resolve the panel, the tool targets the cover panel and says so: a gesture, a screenshot, a fold, a flow step and a recording start carry a warning, a screenshot-diff summary carries a panel line, and a recording stop counts the checks that failed. A gesture completes on the panel it started on, also when the device is folded during it. Unfolded, `rotate` sets the orientation of the device, not of the UI: `Portrait` gives a landscape UI on the inner panel.
