...
| absent  | Argent did not check. This is not evidence of arrival   |

Before Argent reports `false`, it makes one repair where it can prove which characters are
its own: it deletes those characters, then types the text again in small pieces. A
`keyboard` call can therefore change the field by more than the text it adds. A long text
spends tens of seconds in that repair. Where Argent cannot prove which characters are its
own, it leaves the field alone.
...
conclude. The `note` field gives the reason.
