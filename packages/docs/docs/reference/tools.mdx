...
Every tool that acts on a device takes a `udid`: an iOS simulator UDID, an Android serial, a Vega serial, or a `chromium-cdp-<port>` id. The argument is optional. When it is left out, Argent acts on the one booted device that supports the tool, so the common case of a single simulator needs no device id at all - not in the agent's tool calls, and not on the `argent run` command line.

Argent picks a device only when the choice is unambiguous. Each tool declares which platforms it supports, and only booted devices that pass that declaration are candidates: a Chromium tool still resolves with a phone booted alongside the browser, because the phone was never a candidate for it. When no candidate is booted, or when more than one is, the call fails and the error lists the devices, so the next call can name one.
