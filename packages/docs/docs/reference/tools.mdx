...
first keystroke. The Android, iOS simulator and Chromium backends also stop between keys.
A cancel inside an Android repair can leave the field with fewer characters than it held
before the call, because the repair deletes before it types.
