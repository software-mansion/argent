...
Some flags work on macOS only: `argent-lens` and `microinteractions`. `argent flags` marks these flags with `[macOS only]`. On a different platform, `argent enable` stores the flag and exits with code 0. It also prints a note to stderr that the flag has no effect on this host. `argent flags --json` gives the supported platforms in the `platforms` field, or `null` for a flag that works on all platforms.
