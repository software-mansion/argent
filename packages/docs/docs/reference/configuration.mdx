...
Argent reads all `scripts.` keys from the global file only. A project file cannot raise the limits that one script step runs under. Each developer can set `scripts.bash` for their machine.
