...
| ----------------------------- | ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ARGENT_PORT`                 | `3001`                               | The port the tool-server listens on. Set it with `argent server start --port`; a tool-server that Argent starts on demand takes a free port                                                                           |
| `ARGENT_HOST`                 | `127.0.0.1`                          | The address the tool-server binds to. Set it with `argent server start --host`; a tool-server that Argent starts on demand always binds `127.0.0.1`                                                                   |
| `ARGENT_IDLE_TIMEOUT_MINUTES` | `0`                                  | Minutes without a request after which the tool-server exits. Set it with `argent server start --idle-timeout`, where `0` disables the timeout; a tool-server that Argent starts on demand exits after 30 idle minutes |
