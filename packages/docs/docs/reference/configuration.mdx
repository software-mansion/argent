...
| `ARGENT_CHROMIUM_PORTS_FILE`  | `~/.argent/chromium-cdp-ports.json`  | Path of the file that remembers the CDP ports of apps that Argent started; those ports are probed on every `list-devices`, so the env var is not needed to see them |
