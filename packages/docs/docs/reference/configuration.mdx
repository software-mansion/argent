...
| `autoUpdate.enabled`       | global only     | —                | Set to `true` to install eligible Argent updates automatically. Default `false`                                                         |
...
`autoUpdate.enabled` is disabled by default. When it is `true`, the tool-server installs an eligible update after a check. Argent checks at startup and every 24 hours. The update applies to the install that runs the tool-server. Argent still follows the minimum release age policy.

Disable automatic updates with this command:

```bash
argent config set autoUpdate.enabled false
```
