...
| `scripts.env.allow`        | project, global | union            | Additional environment variable names that scripts read from the tool-server. Use an array of names                          |
...
Use `scripts.env.allow` to give scripts access to additional variables from the tool-server's environment:

```bash
argent config set scripts.env.allow '["DATABASE_URL", "AWS_PROFILE"]' --scope project
```

Argent combines the project and global lists. The project's configuration comes from the run's `project_root`, even when the flow file is elsewhere. Argent ignores invalid entries, reserved names and names with the `ARGENT_` prefix. The run reports the entries that Argent ignores.

The tool-server keeps the environment from its start. After you change a shell variable, restart the tool-server. To supply a value for one run, use [Environment values](/docs/reference/flow-yaml#environment-values).
...
Use the same placeholder in script [environment values](/docs/reference/flow-yaml#environment-values), including `argent flow run --env`.
...
For script environment values, `<project-root>` is the run's `project_root`. For `keyboard` and `paste`, it is the tool-server's working directory. To share a secret across these tools, use `~/.argent/secrets.env` or an `ARGENT_SECRET_<NAME>` environment variable.
...
A missing secret fails the step. A missing secret in `--env` or the `flow-execute` `env` parameter prevents the run from starting. Argent hides resolved secrets in failure text and script logs. See [Secrets in an `env` value](/docs/reference/flow-yaml#secrets-in-an-env-value) for the limits. Do not use a placeholder for content that a report must show.
