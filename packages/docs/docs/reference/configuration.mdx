...
| `scripts.heapLimitMb`      | global          | global wins      | The heap limit, in MiB, for `.mjs` flow scripts. Default `512`. Argent refuses a value below `32`. This limit does not apply to Bash                        |
| `scripts.bash`             | global          | global wins      | The absolute path to Bash for `.sh` flow scripts. If unset, Argent selects Bash automatically. See [Bash scripts](/docs/reference/flow-yaml#bash-scripts)   |

Argent reads `scripts.maxTimeoutMs`, `scripts.heapLimitMb` and `scripts.bash` from the global file only. A project file cannot raise the limits that one script step runs under. Each developer can set `scripts.bash` for their machine.
...
If `scripts.bash` is invalid, the `.sh` step reports an error. If Argent cannot read the configuration file, it selects Bash automatically and adds a note to the step report.

A path value is absolute, starts with `~`, or is relative. For `scripts.bash`, use only an absolute path. For `ios.additionalDeviceSets`, Argent resolves a relative entry against the scope that set it: the project root for the project file, the home directory for the global file.
...
argent enable argent-lens --scope project
```
