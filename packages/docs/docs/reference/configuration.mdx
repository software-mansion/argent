...
| `ARGENT_CHROMIUM_PORTS_FILE`  | `~/.argent/chromium-cdp-ports.json`  | Path of the file that remembers the CDP ports of apps that Argent started. Argent probes these ports on every `list-devices`, so `ARGENT_CHROMIUM_PORTS` does not need to repeat them |
