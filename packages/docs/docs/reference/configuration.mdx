...
| `ARGENT_CHROMIUM_PORTS`       | empty                                | Comma-separated CDP ports that Argent probes for Chromium apps, on top of `9222` and the ports `boot-device` itself opened                                                                                                                                                                                                                          |
| `ARGENT_CHROMIUM_PORTS_FILE`  | `~/.argent/chromium-cdp-ports.json`  | Path of the file that remembers the CDP ports of apps that Argent started. Argent probes these ports on every `list-devices` and drops a port from the file as soon as one probe fails, which happens while the app is up with no window. `ARGENT_CHROMIUM_PORTS` is re-read on every call and never pruned, so list a port there to keep it probed |
