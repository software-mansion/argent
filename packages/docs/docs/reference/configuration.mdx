...
A path value is absolute, starts with `~`, or is relative. Argent resolves a relative path in the project file against the project root, and a relative path in the global file against the home directory.

Argent resolves `recordings.directory` on the client host, the machine that receives the mp4.
...
| `ARGENT_AUTO_SCREENSHOT_DELAY_MS`         | per tool, `1400` otherwise | Milliseconds between an interaction tool and its automatic screenshot. See the `disable-auto-screenshot` flag |
