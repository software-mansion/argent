...
| `scripts.env.allow`        | project, global | union            | Extra environment names a flow `script` process inherits from the tool-server, on top of the built-in allowlist. An array of names                                                 |
...
| `scripts.heapLimitMb`      | global          | global wins      | The heap limit, in MiB, that Argent gives to the Node process of a flow `script` step. Default `512`. Argent refuses a value below `32`. The limit does not apply to a bash script |
| `scripts.bash`             | global          | global wins      | The absolute path of the bash that runs a `.sh` script step. See [Bash scripts](/docs/reference/flow-yaml#bash-scripts)                                                            |

Argent reads `scripts.maxTimeoutMs`, `scripts.heapLimitMb` and `scripts.bash` from the global file only. A project file cannot raise the limits that one script step runs under. It also cannot set `scripts.bash`: the value is an absolute path that only one operating system spells that way, so a committed value breaks every `.sh` step for each teammate on another OS. Each developer pins their own bash globally.

Argent reads `scripts.env.allow` from both files, because it names a project input rather than a limit on the host. Argent resolves the project file from the `project_root` of the run, not from the working directory of the tool-server. The working directory of the tool-server is whatever started it, usually your editor, and often `/` or your home directory. So the config comes from the project you ran against, even for a flow kept elsewhere, as in `argent flow run ~/shared-flows/checkout.yaml`.

Argent ignores an entry that names anything starting with `ARGENT_`, whatever its case. That prefix covers its own token, its own port, the address of its own server, and each `ARGENT_SECRET_` value. Argent ignores a name that steers the runner's own process too, and one that is not an environment variable name at all. The run reports each entry it dropped, and names it. Argent ignores `__proto__` as well, although it matches the name rule: Argent copies the environment through a plain object, where that name is an accessor rather than an entry.

Argent drops an entry that is not a string, and a blank entry, when it reads the configuration file. The run reads the file again and names each one it dropped, so a nested list such as `["DB_URL", ["AWS_PROFILE", "AWS_REGION"]]` does not take two names out of every script in silence. `argent config set` echoes back the list it stored, which is where you see it missing. Argent also removes the spaces at each end of an entry it keeps, so `"  DATABASE_URL  "` becomes `DATABASE_URL`. When the whole value is not a list, as in `{ "scripts": { "env": { "allow": "DATABASE_URL" } } }`, Argent reads no names from it and the run says so, naming the file. Without that note the key reads exactly like one you never set.

Pass the value a script needs under a name of your own. The environment of the tool-server is a snapshot from its first start, so a later `export` in your shell does not reach a name on this list. See [Environment values](/docs/reference/flow-yaml#environment-values).

A `scripts.bash` value that Argent reads and cannot use **errors** the `.sh` step. Argent does not fall back to the PATH. A global file Argent cannot read at all is a different case: wrong permissions, or JSON an interrupted write left half-finished. Argent cannot see the key there, so the step runs under the bash on the PATH, and the report of the step says the configuration was not read.

A path value is absolute, starts with `~`, or is relative. The exception is `scripts.bash`: Argent accepts only an absolute path there. For `ios.additionalDeviceSets`, Argent resolves a relative entry against the scope that set it: the project root for the project file, the home directory for the global file.
...
A flow step or a `keyboard` call can use a credential without the credential in the file or in the agent context. The step contains a placeholder:

```yaml
- type: { into: { id: password }, text: "{{secret:APP_PASSWORD}}" }
```

A placeholder also works in an `env` value of a flow `script` step. Four channels accept one: the top-level `env` of a flow file, the `env` of a `script` step, the `env` parameter of `flow-execute` that `argent flow run --env` fills, and the `env` parameter of `flow-add-script`. Nothing is typed there. The tool-server resolves the name and hands the value to the script process. See [Environment values](/docs/reference/flow-yaml#environment-values).
...
`<project-root>` is the `project_root` of the run for a flow `script` step and for the `env` values of the run. A `keyboard` or `paste` call carries no project, so `<project-root>` is the working directory of the tool-server there - whatever started it, often `/` or your home directory. A name that one channel resolves may therefore not resolve in the other. Put the name in `~/.argent/secrets.env`, or export `ARGENT_SECRET_<NAME>` before you start the tool-server, and every channel reads it.
...
A name that no source defines fails the step at run time. In the values of the run, Argent refuses that name before the run starts. Argent replaces a resolved value with its placeholder in the text of a failure, and in the log of a flow `script` step. Argent does not redact any other output. For the exact rule for a `script` step, see [Secrets in an `env` value](/docs/reference/flow-yaml#secrets-in-an-env-value). Thus do not use a placeholder for content that a report must show.
...
| `ARGENT_IOS_TEAM_ID`                                   | unset                                   | The Apple Developer Team ID that signs the runner for [physical iOS devices](/docs/features/physical-ios-devices). Unset, Argent detects the team from the keychain of the Mac. The variable must be in the environment of the tool-server; a value in the shell of the client does not reach a running server |
