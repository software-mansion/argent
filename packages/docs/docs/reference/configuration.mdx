...
| `ARGENT_FFMPEG`               | unset                                | Path or command name of the `ffmpeg` that screen recording uses. Unset, Argent uses the first `ffmpeg` on `PATH` or in the Homebrew and `/usr/bin` locations that has the `libx264` encoder |
