...
1. uploads the runner of Maestro to the simulator, unless the runner is already installed,
2. launches the runner on an ephemeral port,
3. reads the port from the log of the runner,
4. opens a forward tunnel from the local port that Maestro polls to that port,
5. stops the runner and the tunnel when Maestro ends.

The stand-in installs the runner one time per session. Set `SIM_REMOTE_FORCE_RUNNER_INSTALL=1` to install it on every run.

The stand-in writes its steps to the `xctest_runner_*.log` file of Maestro. Read this file when the runner does not start.

## Timeouts

The runner install, the launch, the port discovery and the tunnel must fit in `MAESTRO_DRIVER_STARTUP_TIMEOUT` (default 120 seconds). Increase it on a slow network.
...
- Maestro starts one runner per `maestro test` invocation. A workflow with one invocation per flow file pays the runner launch every time.
