...
`sim-remote` keeps the session token in the memory of the daemon. The daemon writes nothing to disk. There is no configuration file, and the API key is never written to disk.
