...
`sim-remote simctl` runs `xcrun simctl` on the machine. The subcommand names, the arguments, the options, the output and the exit codes are the ones of `simctl`. `sim-remote simctl help [SUBCOMMAND]` shows the usage of `simctl`.
