...
| `SIM_REMOTE_SESSION="<value>" is not a valid session name`     | The session name has characters that `sim-remote` does not accept | Use 1 to 32 lowercase letters, digits, `.`, `_` or `-`, with no leading `.`                                               |
