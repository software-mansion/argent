...
`sim-remote` prints `error: <message>` on stderr. When there is an action to take, a second line `hint: <advice>` follows. The exit code is `1`.
...
**The app in the simulator cannot reach your local server.** The simulator has no route to your `localhost`. Run `sim-remote reverse start <UDID> <PORT>` and confirm the tunnel with `sim-remote reverse status`. See [Tunnels](/docs/cloud/features/tunnels).

**`sim-remote` prints `warning: sim-remote proxy is deprecated`.** A script or Argent uses the old alias. The tunnel works. Update the script to `sim-remote reverse`.
