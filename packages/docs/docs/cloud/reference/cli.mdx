...
| ---------------- | -------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `--server <URL>` | `SIM_ROUTER_URL`     | The URL of the Argent Cloud server. The release binary contains the default. Pass it only if Software Mansion gives you another URL |

## Session commands

| Command                          | Description                                                                                                                                                                                                                                                                                                                                 |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sim-remote login`               | Authenticate and reserve a machine. `--api-key <KEY>` gives the key, or set `SIM_ROUTER_API_KEY`. `--timeout <SECS>` is the maximum wait for a free machine (default `1800`; the server caps it at 3600). `--no-wait` fails at once when no machine is free. `--no-acquire` logs in without a machine. The three options exclude each other |
...
`sim-remote proxy` is a deprecated alias for `sim-remote reverse`. It works and prints a warning on stderr. Update your scripts to `reverse`.
