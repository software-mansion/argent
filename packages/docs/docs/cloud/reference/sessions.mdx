...
Every process reads the variable from its own environment. Set it in the shell or agent that runs the commands, so the shims and every tool that they start use the same session.
