...
The daemon keeps nothing on disk. There is no configuration file, and the API key is never written to disk.
...
`sim-remote` rotates the log file daily and keeps 7 days. Set `RADON_LOG_DIR` to change the directory and `RUST_LOG` to change the filter.
