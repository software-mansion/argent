...
`argent init` and `argent update` check the global directory before they run an install. When npm cannot write there, they print the cause and leave the existing install alone. Update stops before its confirmation prompt. Init offers the ways out below when it can ask. With `--yes`, or no terminal on stdin, it installs the mode the project already recorded, and stops with the remedies when there is none.

Two ways out exist:

- Run `argent init --local`. The project gets `@swmansion/argent` as a devDependency. The project needs no global directory.
- Let Argent point npm at a writable prefix (`~/.npm-global`). Init writes the new prefix to npm's user config file (`~/.npmrc`, unless `npm_config_userconfig` points elsewhere), installs globally, and tells you which directory to add to your `PATH`. Undo the change later with `npm config delete prefix`.
