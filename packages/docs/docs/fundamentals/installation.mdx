...
`argent init` and `argent update` check the global directory before they run an install. When npm cannot write there, they print the cause and leave the existing install alone. Update stops before its confirmation prompt. Init with `--yes` stops with the remedies; without a terminal on stdin it refuses before the check runs, so pass `--yes` to see them. Otherwise init offers the ways out below.
