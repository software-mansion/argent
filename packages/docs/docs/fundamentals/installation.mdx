...
`argent init` and `argent update` check the global directory before they run an install. When npm cannot write there, they print the cause and leave the existing install alone. Update stops before its confirmation prompt. Init offers the remedies below when it can ask. With `--yes`, or no terminal on stdin, it installs the mode the project already recorded. When there is no record, it stops and prints the remedies. It also stops rather than asking when neither of the two remedies it can carry out applies: another package manager, and no `package.json` to add a devDependency to.

The remedies. Init offers the first two as a choice and prints the third:
