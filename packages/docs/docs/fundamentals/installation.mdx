...
`argent init` and `argent update` check the global directory before they run an install. When npm cannot write there, they print the cause and leave the existing install alone. Update stops before its confirmation prompt. Init stops with the remedies whenever it has nobody to ask - `--yes`, or no terminal on stdin. Otherwise init offers the ways out below.
