...
title: Installation
description: Install Argent with npx @swmansion/argent init. The wizard installs the package. It registers the MCP server in your editor. It copies the agent skills into your workspace.
...
pnpm 12.0 to 12.4 stops this command with `ERR_PNPM_IGNORED_BUILDS`. These versions block the build scripts of native dependencies. Do one of these steps:

- Update pnpm to 12.5 or later with `pnpm self-update`. In a terminal, pnpm 12.5 and later ask which packages to build.
- Allow the build scripts of the native dependencies:

```bash
pnpm dlx --allow-build=node-pty --allow-build=tree-sitter --allow-build=tree-sitter-javascript --allow-build=tree-sitter-typescript --allow-build=@fails-components/webtransport-transport-http3-quiche @swmansion/argent@latest init
```
...
Every release on [GitHub](https://github.com/software-mansion/argent/releases) carries the package tarball as an asset, for example `swmansion-argent-0.26.0.tgz`. It is the same file npm serves. Download it and install it globally:

```bash
npm install -g ./swmansion-argent-0.26.0.tgz
argent init
```

Argent's own dependencies still come from your configured npm registry.
...
- **glibc version.** The gesture tools, `screenshot`, `rotate` and `paste` start the bundled `simulator-server` binary. This binary needs glibc 2.39 or newer, for example Ubuntu 24.04 or Debian 13. On an older host, the binary does not start. These tools then fail with ``version `GLIBC_2.39' not found``. Run `ldd --version` to see the glibc version of the host.
