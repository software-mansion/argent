...
Argent drives the iPhone Duo simulator closed, half-open and open. The agent folds the device with one tool, and every screenshot, element tree, tap and recording follows the panel the device renders to. See the `fold` tool in the [tools reference](/docs/reference/tools).
