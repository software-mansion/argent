...
Xcode 27 replaces Simulator.app with DeviceHub. DeviceHub connects to every simulator that starts. That connection moves the simulator input devices to CoreDevice. Taps, hardware buttons and typed text from Argent then stop reaching the simulator. Argent reports no error.

Quit DeviceHub and start the simulator again to restore input.
