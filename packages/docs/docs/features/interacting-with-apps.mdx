...
The agent also clears a text field that already holds a value. The agent clears the field, then types the new value. Thus the agent replaces a value instead of adding to it. On a mobile target, the agent removes the text before the cursor and the text after it. Thus a field with many lines also becomes empty. On Chromium, the agent selects the field and deletes the selection, then reads the field again to make sure it is empty.
...
- On mobile targets, the clear action removes up to 100 characters on each side of the cursor. A field with more characters on one side of the cursor keeps the remainder, and the agent clears it again. On Chromium there is no limit.
- On Chromium, the agent reads the field again after the clear, so it knows that the field is empty. On mobile targets the agent knows only what it sent, so it reads the field itself when it needs proof.
- The clear action is not available on TV targets and Vega. There the agent uses the on-screen keyboard of the app.
