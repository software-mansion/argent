...
- On Chromium, the agent reads the field again after the clear, so it knows that the field is empty. On the other targets the agent knows only what it sent, so it reads the field itself when it needs proof.
