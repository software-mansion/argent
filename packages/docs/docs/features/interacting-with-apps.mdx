...
- On Chromium, Argent uses the optional `sharp` package to scale and to rotate a screenshot. Install `sharp` in the environment of the tool-server before you use `scale` or `rotation` there. Without `sharp`, Argent returns the capture unchanged and reports no error.
