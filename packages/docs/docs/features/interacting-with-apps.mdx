...
that burst. The device still reports success. Argent finds the loss. Argent then retries in
small pieces, but only where it can prove which characters are its own. Argent reports
whether the text landed. The report gives counts, not the contents of the field. See the
