...
that burst. The device still reports success. Argent finds the loss. Argent then repairs the
field in small pieces, but only where it can prove which characters are its own. Argent reports what it
could confirm. The report gives counts, not the contents of the field. See the
