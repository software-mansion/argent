...
On an Android phone or tablet, Argent reads the field back after it types. Android injects
the text as one burst of key events. A field that redraws on each keystroke drops part of
that burst. The device still reports success. Argent finds the loss when it can read the
field back. Argent then repairs the field, but only where it can prove which characters are its own: it
deletes those characters first, then retypes them in small pieces, so a repair cut short can
leave the field holding less than before the call. Argent reports what it could confirm. The
report gives counts, not the contents of the field. See the
[tools reference](/docs/reference/tools#typed-text-on-android).
