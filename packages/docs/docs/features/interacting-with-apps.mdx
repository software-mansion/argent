...
The agent also sets device-wide settings without opening the Settings app. Dark mode, text size and the accessibility toggles work on iOS simulators and Android. The radios, location and auto-rotate are Android only. This makes visual tests repeatable: the agent pins the theme and text size it wants up front, rather than trusting whatever the device was left in. The setting persists, so the agent sets it back when a later step needs a different state.
...
- Device-wide settings cover iOS simulators and Android; the radios, location and auto-rotate are Android only.
