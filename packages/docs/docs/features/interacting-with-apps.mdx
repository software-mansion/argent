...
The agent opens an app by its bundle id or package name. The agent does not tap an icon on the Home screen. Thus each run starts from a known state. The agent also opens deep links by URL, restarts an app (iOS, Android, and TV — not a Chromium app), and reinstalls an app with clean data.
