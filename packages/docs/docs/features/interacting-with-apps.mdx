...
- A screen can have very many elements. Argent then shows 500 of them: the first 400 and the last 100. Argent also gives the number of the elements that it does not show. The agent scrolls the missing part into view to read it. The agent can also wait for such an element, because the selector tools match against the full tree.
