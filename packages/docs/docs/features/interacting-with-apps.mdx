...
The agent also controls the device around the app. The agent rotates the device between portrait and landscape. The agent folds and unfolds a foldable iOS simulator, and its screenshots, element trees and taps follow the panel the device shows. The agent presses hardware buttons such as home, back and volume. On TV targets, the agent presses the remote. The agent shakes the device to start the handler that your app binds to a shake, for example a feedback sheet or the developer menu.
