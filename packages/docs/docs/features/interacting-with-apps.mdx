...
The agent also clears a text field that already holds a value. The agent clears the field, then types the new value. Thus the agent replaces a value instead of adding to it. On a mobile target, on Apple TV and on Android TV, the agent removes the text before the cursor and the text after it. Thus a field with many lines also becomes empty. On Vega the agent removes only the text before the cursor, because the platform has no forward delete. On Chromium, the agent selects the field and deletes the selection, then reads the field again to make sure it is empty.
...
- "Clear the name field and type `Ada Lovelace`."
...
- On mobile targets, Apple TV and Android TV, the clear action removes up to 100 characters on each side of the cursor. A field with more characters on one side of the cursor keeps the remainder, and the agent clears it again. On Chromium there is no limit.
- On Vega the clear action removes up to 200 characters before the cursor and none after it, because the platform has no forward delete.
- On Chromium, the agent reads the field again after the clear. When that read finds the field empty, the agent knows the field is empty. A page can replace the field or stop showing its value; then the read gives no answer, and the agent knows only what it sent. On the other targets the agent always knows only what it sent, so it reads the field itself when it needs proof.
- On a remote Apple TV the clear action is not available, because the TV tools drive a simulator on the machine that runs Argent. There the agent uses a local Apple TV simulator, or the on-screen keyboard of the app.
