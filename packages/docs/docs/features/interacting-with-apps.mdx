...
The agent does not read coordinates from a screenshot. Before each tap, the agent reads the elements on the screen with their roles, labels and positions. The source depends on the app:
...
On Android, the agent also reads the content of a `WebView`. Chromium puts the web page into the accessibility tree. Thus the agent gets the elements of the page and the native elements around them in one tree. Some WebView versions also send the HTML `id` of an element. The agent then selects a web control by id, in the same way as a native control. Other WebView versions send no id. The agent then selects the control by its text or its role. Argent also removes a layout element of the page that has no name. A `<div>` or a `<form>` is such an element. Argent removes its `id` with it. The agent selects the controls in the element instead.

The renderer needs a moment to publish the page. Before it does, the agent sees the `WebView` and no content in it. The agent waits for an element of the page, then continues.

After a screen change, the agent reads the screen again. Most actions return a screenshot, so the agent sees the result without an extra call.
...
- A screen can have very many elements, or very much text. Argent then shows 500 elements or 40,000 characters, whichever limit comes first. It keeps the start and the end of the screen and drops what is between them. Argent also gives the number of the elements that it does not show. The agent scrolls the missing part into view to read it. The agent can also wait for such an element, because the selector tools match against the full tree.
- On Android and Chromium, a screen can hold more elements than one capture carries. The agent then gets a part of the tree. Android always sends a message that says so. Chromium sends the message for a page with too many elements. Chromium sends no message for a page that is too deep. The other platforms have no such limit and send no such message.
