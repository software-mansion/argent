...
A busy app does not answer the accessibility read. Argent then marks the read as `unreadable` instead of returning an empty tree, and the wait tools judge stillness from screen captures. The agent waits for the app to settle and reads the screen again.
