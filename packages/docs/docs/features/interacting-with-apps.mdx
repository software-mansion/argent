...
The agent also sets device-wide settings without opening the Settings app. Dark mode, text size and the accessibility toggles work on both platforms. The radios, location and auto-rotate are Android only. This makes visual tests repeatable, because each run starts from a known theme and text size.
