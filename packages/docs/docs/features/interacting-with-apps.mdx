...
that burst, and the device still reports success. Argent finds this, retries once in small
pieces where it can prove which characters are its own, and reports whether the text
landed. The report gives counts, not the contents of the field. See the [tools
