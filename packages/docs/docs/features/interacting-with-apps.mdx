...
The agent also sets device-wide settings without opening the Settings app: dark mode, text size and the accessibility toggles on both platforms, and on Android the radios, location and auto-rotate. This makes visual tests repeatable, because each run starts from a known theme and text size.
