...
After a screen change, the agent reads the screen again. Each action returns a screenshot, so the agent sees the result without an extra call.
...
The agent grants, denies or resets a runtime permission such as location, camera or notifications. The agent does not navigate the Settings app. Then the agent launches the app again and sees the permission prompt. Thus a first-run permission flow is repeatable.
...
The [tools reference](/docs/reference/tools) lists the tools in the **Interacting** and **Devices and apps** groups with their parameters.
