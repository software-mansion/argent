...
Argent stores each flow as a YAML file in the `.argent/flows/` directory of the project. The file contains a list of steps. The file does not contain a device identifier. Thus the same flow runs on a different simulator or a different emulator. A flow can also name the targets that it supports, for example a phone or a TV. Argent runs the flow only on a target of that kind. In a run over a whole directory, Argent skips the flow on every other target.
...
A flow can contain more than taps and text. A step can wait for an element, wait for the screen to settle, check a condition, take a screenshot and compare it to a baseline, or run a different flow. See [Baselines in flows](/docs/features/visual-regression#baselines-in-flows) for the screenshot comparison.
