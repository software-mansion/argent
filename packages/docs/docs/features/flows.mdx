...
A script reads its inputs from its environment. The flow file holds the defaults, and `argent flow run --env NAME=value` holds what changes per run, so one flow serves a local run and a CI job. Write a credential as `{{secret:NAME}}`. Argent resolves it on the machine that runs the flow, and replaces the value with the placeholder in the failure text and the log of the step. See [Environment values](/docs/reference/flow-yaml#environment-values).
