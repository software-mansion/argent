...
A script reads its inputs from its environment. The flow file holds the defaults, and `argent flow run --env NAME=value` holds what changes per run, so one flow serves a local run and a CI job. Write a credential as `{{secret:NAME}}`, which Argent resolves on the machine that runs the flow and keeps out of the failure text and the log of the step. See [Environment values](/docs/reference/flow-yaml#environment-values).
