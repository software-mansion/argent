...
Use a local script for setup or cleanup that device steps cannot do. A script is an `.mjs` file or a `.sh` file. Ask the agent to add the script at the required point in the flow.
