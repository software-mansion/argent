...
- The agent records a launch step for iOS and Android apps. For a Chromium or Electron app, add the launch step to the file by hand. The guided agent workflow covers iOS, Android, and Chromium at this time. The runtime still replays a Vega flow (`argent flow run --platform vega`), and the [Flow YAML reference](/docs/reference/flow-yaml) documents the Vega step forms.
