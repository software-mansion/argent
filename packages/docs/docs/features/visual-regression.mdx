...
| Part           | Content                                                                                                                                                                                           |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Overall status | `unchanged`, `changed`, `resized_no_change` or `dimension_mismatch`. The first three also report the percentage of pixels that are different. `dimension_mismatch` reports the two sizes instead. |
...
Argent writes a diff image for each comparison that runs. The image marks the pixels that became brighter, the pixels that became darker, and the outline of each changed area. The agent shows you this image when you ask for it. Argent writes no diff image on the status `dimension_mismatch`, because Argent compares nothing there.
...
- The two screenshots must come from the same device class and the same orientation.
- Argent does not compare two screenshots with different aspect ratios. Argent reports the status `dimension_mismatch`.
- When only the resolution differs, Argent scales the larger screenshot down to the size of the smaller one. Argent then reports a clean result as `resized_no_change` and not as `unchanged`.
- The scaling can add a small difference. The scaling can also remove one, for example a hairline or a change in the subpixel text. A clean `resized_no_change` is thus weaker evidence than a clean `unchanged`.
...
- The comparison works on an iOS simulator, or an Android emulator or device. It does not work on Chromium, Apple TV or Vega apps. On a [physical iPhone](/docs/features/physical-ios-devices), the agent captures full-resolution screenshots and compares them on the host instead.
