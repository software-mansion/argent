...
A [flow](/docs/features/flows) can contain a snapshot step. The first run of the flow stores a baseline of the screen. Each later run compares the screen to the stored baseline. The step fails when the difference is above a tolerance. A snapshot step can also compare one element only instead of the full screen.
