...
- Argent starts the capture at one of these three moments:
  - JS layer: the agent reads the network log for the first time.
  - Chromium: Argent attaches to the Chromium app.
  - Native iOS: you activate the native inspection.
- Argent does not capture the earlier requests. Start the capture, then do the operation again.
- In React Native, a reload of the JavaScript code removes the capture and all captured requests.
- If the JS layer has no requests, Argent tells the agent how long the capture has been active. A short time does not show that the app sent no requests.
