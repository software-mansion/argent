...
The agent launches, restarts and reinstalls apps, reads the accessibility tree, takes screenshots, compares screenshots for visual regressions, taps and double-taps, swipes, long-presses and drags, presses the hardware buttons, types text, waits for elements, and records and replays [flows](/docs/features/flows).
