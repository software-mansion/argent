...
2. The agent renders each variant in the app.
3. The agent stages each variant with the `propose_variant` tool while the variant is on screen. The tool screenshots the device for the preview image. The agent does not block. The Lens window opens on your screen.
