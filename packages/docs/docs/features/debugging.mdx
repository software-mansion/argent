...
- If the app's JS runtime dies after the app logged something, Argent keeps the log file of that session. The agent gets the path of that file in a note, and reads the file to see the logs from before the crash. The note says that no file was kept when Argent could not create one, usually because `~/.argent/tmp` is not writable, or when something removed it afterwards. Argent removes the file at the next debugger session that finds it one day old. A second crash on the same device can remove it earlier.
