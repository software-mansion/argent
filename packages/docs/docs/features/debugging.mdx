...
The agent reproduces a failed state. The agent restarts the app, reloads the Metro bundle, grants or resets a permission, or opens a URL. Of these, only opening a URL survives on a Chromium app: the user quits it, and the agent starts an Electron app again itself. Then the agent does the steps again, and reads the logs and the component tree after each step.
