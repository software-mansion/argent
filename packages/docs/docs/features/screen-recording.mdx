...
- Some simulator-server builds record video without an extra tool. Every other build encodes with `ffmpeg` instead, and a failed recording names it. Install `ffmpeg` only when a recording asks for it, not up front. The build, not the platform, decides which applies.
