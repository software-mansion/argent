...
- Where simulator-server can record for itself, no extra tool is needed. On every other build Argent encodes the video with `ffmpeg` instead, and the failed recording names it — install `ffmpeg` when a recording asks for it rather than up front. Which one applies is a property of the build, not the platform.
