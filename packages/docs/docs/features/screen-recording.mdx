...
The video has one size: the size of the screen at the start of the recording. When the screen changes size during the recording, Argent fits the new screen into the video and adds black bars. Argent does not stretch the screen. For example, the screen changes size when a foldable simulator changes panels.
