...
const TOOL_IDS_SRC_DEST = path.resolve(__dirname, "../src/generated/argent-tool-names.json");
...
// Generate canonical tool ids from the registry so config adapters don't need
// to maintain a parallel list by hand.
...
try {
  ...
  fs.mkdirSync(path.dirname(TOOL_IDS_SRC_DEST), { recursive: true });
  fs.writeFileSync(TOOL_IDS_SRC_DEST, JSON.stringify(toolIds, null, 2) + "\n");
