// Keep this list alphabetized. mcp-configs.test.ts verifies it stays in sync
// with the live tool registry, while keeping the runtime dependency surface
// here small and package-friendly.
export const CODEX_APPROVED_TOOL_NAMES = [
  "boot-simulator",
  "button",
  "debugger-component-tree",
  "debugger-connect",
  "debugger-evaluate",
  "debugger-inspect-element",
  "debugger-log-registry",
  "debugger-reload-metro",
  "debugger-status",
  "describe",
  "dismiss-update",
  "flow-add-echo",
  "flow-add-step",
  "flow-execute",
  "flow-finish-recording",
  "flow-read-prerequisite",
  "flow-start-recording",
  "gather-workspace-data",
  "gesture-custom",
  "gesture-pinch",
  "gesture-rotate",
  "gesture-swipe",
  "gesture-tap",
  "ios-profiler-analyze",
  "ios-profiler-start",
  "ios-profiler-stop",
  "keyboard",
  "launch-app",
  "list-simulators",
  "native-describe-screen",
  "native-devtools-status",
  "native-find-views",
  "native-full-hierarchy",
  "native-network-logs",
  "native-user-interactable-view-at-point",
  "native-view-at-point",
  "open-url",
  "profiler-combined-report",
  "profiler-commit-query",
  "profiler-cpu-query",
  "profiler-load",
  "profiler-stack-query",
  "react-profiler-analyze",
  "react-profiler-component-source",
  "react-profiler-cpu-summary",
  "react-profiler-fiber-tree",
  "react-profiler-renders",
  "react-profiler-start",
  "react-profiler-stop",
  "reinstall-app",
  "restart-app",
  "rotate",
  "run-sequence",
  "screenshot",
  "stop-all-simulator-servers",
  "stop-metro",
  "stop-simulator-server",
  "update-argent",
  "view-network-logs",
  "view-network-request-details",
] as const;
