...
- **Debugger Connection** - if the debugger connection is interrupted (closed), the started profiling will also close, making the `profiler-stop` command unsuccesful. When encountered errors - check the debugger status and try to restart the profiling flow.
