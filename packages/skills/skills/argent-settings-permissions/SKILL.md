...
- The app must **declare** the permission in its manifest. Undeclared mapped permissions come back in `skipped`; a `grant` errors only if none took effect, while a `deny`/`reset` of an undeclared permission is already satisfied.
...
- **A partial Android result is normal.** `applied` lists what actually changed; `skipped` lists mapped permissions that did not take effect (usually not in the manifest). Both together tell you what happened.
...
- `unverified` - Android only, applied entries that could not be confirmed.
