...
- `udid` - target device (iOS simulator UDID, or Android serial). Optional: with one booted device on a platform the tool declares, the server resolves it. Pass it to pick one of several; `list-devices` shows what is booted. See `argent-ios-simulator-setup` / `argent-android-emulator-setup` for the full setup flow.
