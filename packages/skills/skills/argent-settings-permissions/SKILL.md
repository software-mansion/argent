...
| The setting isn't one of the **11 runtime permissions** below (e.g. Wi-Fi, cellular data, dark mode, VPN, Focus) | **Do NOT use this tool**                                                                      | Out of scope. Dark mode, text size, increase contrast, reduce motion, invert colors, and (Android) Wi-Fi, cellular data, airplane mode, the device-wide location switch and auto-rotate belong to `system-settings` - see `argent-device-interact`. For anything neither tool covers, drive the Settings app or the app's own UI. |
