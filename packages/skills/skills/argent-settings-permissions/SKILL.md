...
| The setting isn't one of the **11 runtime permissions** below (e.g. Wi-Fi, cellular data, dark mode, VPN, Focus) | **Do NOT use this tool**                                                                      | Out of scope. Dark mode, text size, reduce motion, invert colors, and (Android) Wi-Fi, cellular data, airplane mode, location and auto-rotate belong to `system-settings` - see `argent-device-interact`. For anything neither tool covers, drive the Settings app or the app's own UI. |
