...
Both need a **Debug** build + Metro running. argent only *connects* to Metro — it does **not** start Metro or port-forward (any platform); do these in your shell:
...
- `gesture-*` (use `remote`), `describe` (no a11y tree), `open-url` (not wired) → all return "unsupported on vega".
