...
For **Vega (Fire TV)**: requires a **Debug `.vpkg`** installed on the VVD, **Metro running**, and the device able to reach it (`vega device start-port-forwarding --port 8081 --forward false`). Confirm `http://localhost:8081/json/list` shows a `Hermes React Native` target. Vega's React Native is a fork of RN 0.72, which serves the _legacy_ Hermes inspector: `debugger-connect`, `debugger-status`, `debugger-evaluate`, `debugger-log-registry`, `view-network-logs`, and `view-network-request-details` all work (they need only `Runtime.evaluate`, and the network inspector patches `fetch` rather than using CDP `Network`). `debugger-component-tree`, `debugger-inspect-element`, `debugger-reload-metro`, and the `react-profiler-*` tools are rejected at the capability gate — they deliver results over `Runtime.addBinding`, which that Hermes acknowledges but never implements. Use `describe` for on-screen structure on Vega instead. `debugger-status` reports `isNewDebugger: false` and an empty `projectRoot` there (RN 0.72's Metro sends no project-root header), so component `file:line` source mapping is unavailable.
