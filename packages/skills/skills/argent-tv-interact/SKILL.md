...
- `keyboard {udid, text}` — type into the focused field (focus it with `tv-remote` first). Named `key` presses (e.g. `{key:"enter"}`) work on Vega; on Apple TV / Android TV move focus with `tv-remote` instead. `clear: true` is **rejected on every TV target and on Vega** — empty a field with repeated `{key:"backspace"}` on Vega, or the app's own clear affordance on a TV (where `key` is rejected too).
