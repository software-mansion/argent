...
**Setup (before Step 1):** if the run needs a specific device state — dark mode, a larger text size, an accessibility toggle, the Android radios — pin it with `system-settings` (see `argent-device-interact`) **before** the baseline screenshot, so the baseline and every later screenshot share one state. To take a permission prompt out of the flow, use `argent-settings-permissions` here too.
