...
**Before testing, resolve which device to test on.** With one device booted the tools resolve it themselves and you can omit `udid` entirely. Otherwise follow `<device_selection_rule>`, reading `list-devices` to choose: prefer a running device on any platform;
