...
`"momentum"` defaults to `true` (a natural flinging swipe). Pass `"momentum": false` for a momentum-free swipe: the finger decelerates into the end point, so the OS reads ~0 release velocity and applies little to no fling. Use it when you need a deterministic scroll distance, e.g. scroll-to-element loops.
