...
- `text` condition also needs `expectedText` (substring the matched element must contain, or the whole string with `textMatch: equals`). Folded the same way as `text` above; a leading or trailing space is significant, and a whitespace-only value matches nothing.
