...
> **TV targets (Apple TV / Android TV) are not covered by this skill.** A TV target is **focus-driven, not touch-driven** — the `gesture-*` tools are the wrong tools for it, and `button` / `keyboard` behave differently on a TV (remote presses / text into the focused field) than the touch-oriented usage described here. This applies to both Apple TV simulators (UUID-shaped, identical to iOS) and Android TV / leanback devices (serial-shaped, identical to a phone emulator). If `list-devices` tags your target `runtimeKind: "tv"`, stop and use the `argent-tv-interact` skill: `describe` to read focus, `button` for remote presses, `keyboard` to type, and `tv-set-focus` to jump focus by label.
