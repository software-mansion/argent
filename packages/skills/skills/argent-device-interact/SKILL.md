...
Keep the tap and the clear in ONE `run-sequence`, as above. One tool-server is shared by every agent session on the machine, so another session can move focus on this device at any moment — with a tap, a `launch-app`, a `button` or an `open-url`. A sequence that uses the keyboard holds that device's keyboard for all of its steps, so no other session's `keyboard` or `paste` runs between your tap and your clear. Two separate calls have no such hold, and the clear then goes wherever focus is by the time it runs.

On iOS and Android the clear is a burst of delete keys in both directions, so it does not matter where the tap left the cursor and a multi-line field empties too — but it is bounded per side, not in total: 100 characters behind the caret and 100 ahead of it. A tap into a filled field usually lands at the END, where only the backspaces do anything, so one call clears 100 characters, not 200 — a 250-character field measurably keeps 150. Call `clear` again until the field reads empty. Chromium instead selects the focused editable and deletes the selection, with no length limit.

`cleared` means two different things, and `clearVerified` is what tells them apart: on Chromium the field is read back, and `clearVerified: true` is added when that read found it EMPTY; on iOS and Android nothing is read back, the result reports what was SENT, and `clearVerified` is absent. It is absent on Chromium too where the field could not be read back at all — a page that replaced it, sealed `window`, or left it on screen with nothing readable on it. Branch on that rather than on `keys` (200 on iOS/Android, 0 on Chromium, which sends no key events). Where it is absent, `describe` the field or check what the app does with it if you need proof. Not supported on TV targets or Vega; there, empty the field through the app's own on-screen keyboard with `tv-remote`.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `KEYBOARD_CLEAR_NO_EDITABLE_FOCUS` | Chromium only. Nothing editable holds focus, or the whole document is the editing host. Nothing was cleared and nothing was selected                                                                                                                          | Tap the field (`gesture-tap`), then clear it. If the same error returns, the field may be `disabled`: a disabled control cannot take focus at all                                                                                                                                                                  |
| `KEYBOARD_CLEAR_UNSUPPORTED_FIELD` | Chromium only. The clear reached an element it cannot empty: a date/time input, a readonly, disabled or non-text control, an opaque custom element, an `<iframe>`, a page that broke the script, an editor that restored the value, or a mask that rewrote it | Do not retry the same call — the message says which of these it was. Press `key: "backspace"` on a date/time input; for the rest, `gesture-drag` to select the text and type over the selection, or use the app's own control. Where a mask rewrote the value, read the field first: the old value is already gone |
| `KEYBOARD_CLEAR_UNCONFIRMED`       | Every platform. The clear was delivered and the outcome is unknown: on Chromium the renderer did not answer or the connection dropped, on iOS and Android the key transport died mid-burst and the field may be PARTIALLY emptied                             | Read the field back (`describe`) before you clear or type again, then act on what it holds. Never retry blind: a second clear on a field the first one emptied is not a no-op for the app                                                                                                                          |
| `KEYBOARD_TARGET_KIND_UNKNOWN`     | iOS and Android. Argent could not tell a phone/tablet from a TV, and the two mean different things by `clear`. Nothing was sent                                                                                                                               | Check `list-devices` reports the device ready, then retry. If it IS a TV, empty the field with the app's own on-screen keyboard, driven with `tv-remote`                                                                                                                                                           |
