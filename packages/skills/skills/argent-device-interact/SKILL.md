...
- `index`: when several match, which one to act on. For read (`get-text`/`get-attrs`) and `wait` it is `0 = topmost in reading order`; for a **tapping** action (`tap`/`focus`/`type`/`fill`) an enclosing container is ranked *after* the smaller matches it wraps, so `0` is the innermost/most-specific match (this is what makes `find` pick a child input over the card that folds its text on Android). `exists` is a pure presence check and **ignores `index`** (it reports whether *any* element matches, and its `match` is always the topmost visible one). `matchCount` counts the matches `index` can address — **visible** matches for the acting actions and `wait`, **all** matches for `get-text`/`get-attrs`/`exists` — so if it is `> 1`, narrow `query` or set `index`.
