...
- `text` condition also needs `expectedText` (substring the matched element must contain, or the whole string with `textMatch: equals`). Folded the same way as `text` above; a value of only invisible characters matches nothing.
