...
> **TV targets (Apple TV / Android TV) are not covered by this skill.** A TV target is **focus-driven, not touch-driven** — the `gesture-*`, `button`, and `keyboard` tools are the wrong tools for it. This applies to both Apple TV simulators (UUID-shaped, identical to iOS) and Android TV / leanback devices (serial-shaped, identical to a phone emulator). On Apple TV these tools are blocked; on Android TV they technically execute (it's still an Android device) but do nothing useful on a focus-driven UI. Either way, if `list-devices` tags your target `runtimeKind: "tv"`, stop and use the `argent-tv-interact` skill (`tv-describe` / `tv-navigate` / `tv-set-focus` / `tv-type`) instead.
