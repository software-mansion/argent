...
- Returns `{ found, matchCount, match?, actionResult?, elapsed, note? }`. `found: false` carries a `note` explaining why (no match / not visible / index out of range / degraded read). It does **not** throw, so still confirm `found` before assuming the action ran. For `exists`, a `presenceUnknown: true` flag means the tree couldn't be read at all, so `found: false` is "couldn't tell", not "confirmed absent".
