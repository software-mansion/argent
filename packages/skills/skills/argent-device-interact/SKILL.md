...
On iOS and Android the clear is a burst of delete keys in both directions, so it does not matter where the tap left the cursor and a multi-line field empties too — but it is bounded per side, not in total: 100 characters behind the caret and 100 ahead of it. A tap into a filled field usually lands at the END, where only the backspaces do anything, so one call clears 100 characters, not 200 — a 250-character field measurably keeps 150. Call `clear` again until the field reads empty. Chromium instead selects the focused editable and deletes the selection, with no length limit. Chromium refusals come in two shapes, and the error names which. **Tap the field and retry** for `KEYBOARD_CLEAR_NO_EDITABLE_FOCUS`: nothing editable holds focus, or the whole document is the editing host. Retrying that same tap is pointless for `KEYBOARD_CLEAR_UNSUPPORTED_FIELD`, where the focused element is already the one you meant — a date/time input (`date`, `datetime-local`, `month`, `week`, `time`), whose structured value a select-and-delete cannot remove (clear one of those with `keyboard` `{ "key": "backspace" }` while it has focus); a readonly, disabled or non-text control; a custom element exposing no open shadow root, which is refused rather than deleted blind; a rich-text field holding a `contenteditable="false"` block; an editor that restores the value it just deleted; or a field whose page rewrote the value into its own (a currency or phone mask) — there the old value is already gone, so read the field before typing. A third, `KEYBOARD_CLEAR_UNCONFIRMED`, means the renderer never answered: do not retry blind, read the field back first. For all of these, `gesture-drag` to select the text and type over the selection is the fallback that works. `cleared` means two different things, and `clearVerified` is what tells them apart: on Chromium the field is read back, and `clearVerified: true` is added when that read found it EMPTY; on iOS and Android nothing is read back, the result reports what was SENT, and `clearVerified` is absent. It is absent on Chromium too in the rare case the field could not be read back at all — a page that replaced it, or sealed `window`. Branch on that rather than on `keys` (200 on iOS/Android, 0 on Chromium, which sends no key events). Where it is absent, `describe` the field or check what the app does with it if you need proof. Not supported on TV targets or Vega; there, empty the field through the app's own on-screen keyboard with `tv-remote`.
