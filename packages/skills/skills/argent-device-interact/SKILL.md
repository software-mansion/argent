...
On an Android phone or tablet the typed text is read back off the screen, because `adb input text` injects it as one key-event burst that a field re-rendering per keystroke silently drops part of. The result then carries `verified` and, when there is anything to caveat, a `note`: `true` means the field really holds the text; `false` means the read-back did not confirm it — the field was read and does not hold the text, the retry itself failed partway, or the retry ran and the read that would have confirmed it could not be taken. That retry backspaces what the call can prove is its own and retypes it in smaller chunks, so a `keyboard` call may modify the field beyond appending, and a long text spends tens of seconds there. `verified` is **absent** whenever the check could not conclude, and on every other platform; absent means "not checked", never "checked and fine" — and never that the field is untouched, because the retry can run and the read that follows it can still conclude nothing. A `false` verdict STOPS the surrounding `run-sequence` (so the Enter step above does not fire), FAILS a flow `type` step or a flow `tool: keyboard` step, and makes the flow recorder warn about the step it just recorded, including when the recorded call is the `run-sequence` that holds the typing. A flow `type` step that passes with a `note` carries it as the step's warning, so an unverified pass is visible in the report. Read `note` before retrying: it says whether anything was retyped.
