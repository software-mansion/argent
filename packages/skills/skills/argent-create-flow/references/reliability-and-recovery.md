...
Keep the source screen available and do these checks:
...
An unavailable tree makes the candidate test void. It never authorizes coordinates. Relevant failures include unavailable iOS native devtools, an unreachable Android helper, an unreachable Chromium CDP session, or missing Vega page source. Restore the tree and repeat the test.
...
2. If the warning repeats, call `native-devtools-status` with the same UDID and bundle id.
3. If `requiresRestart` is true, restart once and check again.
4. If an injectable app remains disconnected, call scoped `stop-all-simulator-servers` once. This does not change app or account data. Then restart and check status again.
...
A generic system-app flow can use:
...
For a normally injectable app, raw `restart-app` can produce a self-resetting fragment. It is not e2e and cannot complete `argent-qa-flows`.
...
| Environment error  | The check could not run                        | Repair the environment. Do not count an app verdict       |
...
| Idle warning       | A readiness step passes without settling       | Inspect intended motion versus incomplete loading         |
