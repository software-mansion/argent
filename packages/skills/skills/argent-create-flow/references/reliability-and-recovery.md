...
| Debugger note      | A debugger step passes carrying a note         | Read it in this report. A [reaped-session record is spent by the read](flow-yaml.md#directives), so no rerun repeats it - grep the log file where it names one. A note about this session's OWN missing file is spent by nothing and returns every run until a new session mints a writer |
