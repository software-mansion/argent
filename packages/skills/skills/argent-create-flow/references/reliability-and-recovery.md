...
| Chromium | No page to drive, or no reachable CDP session | Call `debugger-status` and follow the `guidance` on its result — it routes on the `detail`. The quit is the user's move: `boot-device` only starts an app and never stops one, and an app that merely lost its window needs a window, not a relaunch |
