...
The CLI and the MCP block word it differently, and only the CLI marks the indeterminate case — so on the MCP block a stale, last-trusted tree reads exactly like the screen that failed. Check `determinacy` there rather than the `screen:` wording.

| CLI `screen:` line               | MCP `screen:` line                                          | Which read                                                                |
| -------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------- |
| `47 elements, 390x844`           | `47 elements, 390x844, captured at the failure, via <src>`  | the tree the failing step itself read — the screen that failed            |
| `… (captured after the failure)` | `…, captured AFTER the failure — the app may have moved on` | the step held no tree, so the runner read one straight after it failed    |
| `… (last trusted read)`          | (no marker — read `determinacy` instead)                    | an indeterminate failure — the screen went unreadable before the deadline |
| `unavailable — <reason>`         | `unavailable (<reason>) — <detail> [<hint>]`                | there is no element list at all; the reason says why                      |

`screenshot` and `tree` are optional slots, not guarantees. Most of the indeterminate family carries no `tree` — `condition-dark-tail` is the exception: it attaches the last trusted read, so it renders a full element list. And no `screenshot` is captured once a `{{secret:…}}` value has been typed onto the device: pixels are never masked, so the capture is declined for that device from then on, not just for the run that typed it. A failed `snapshot:` still carries its own `baseline`/`current`/`diff` images on the step's artifact lines, and `--output` exports them — review those before publishing them as CI artifacts.

A `screenshot:` line reading `(omitted — …)` or `(unavailable — …)` is prose, not a path: the reason says whether argent declined the capture, had no screen to capture, or asked for one the device did not return.
...
A code from a newer tool-server that isn't listed here reads like `unclassified`: work from `message`, `hint`, and `cause`.

| `failure.code`                                                                                                                                                      | What it means                                                                                                         | Strategy                                                                                                                                                                                                                                                                                                                                                           |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `selector-not-found`, `selector-not-visible`, `selector-scope-unresolved`                                                                                           | nothing matched / everything that matched had a zero-area frame / a `within`/`after`/`next` scope resolved to nothing | **Edit the selector** from the ranked `candidates`. For `-not-visible` the element is already in the tree with a zero-area frame, so neither `scroll-to` nor `await` can help — find out why it has no size (behind an overlay, collapsed, not laid out yet). For `-scope-unresolved`, fix the scope's anchor first — the target was never looked for.             |
| `target-missing`                                                                                                                                                    | the gesture step named neither a selector nor `x`/`y`                                                                 | Give the step a target. There is no `candidates` list here — nothing was ever searched for.                                                                                                                                                                                                                                                                        |
| `text-mismatch`, `text-no-match`                                                                                                                                    | the element was found; its text is not what the flow asserted (`expected` / `actual` show both)                       | Either the app changed or the assertion is stale — update the expected text, or report the app bug. Never loosen an assertion just to make it green.                                                                                                                                                                                                               |
| `assert-hidden-unmet`                                                                                                                                               | something asserted `hidden` was still on screen                                                                       | Check that the step meant to dismiss it ran; if it is a race, gate with `await: { hidden: … }` before the assert.                                                                                                                                                                                                                                                  |
| `condition-never-readable`, `condition-dark-tail`, `condition-hidden-unconfirmable`, `when-guard-indeterminate`, `tree-source-not-ready`, `tree-source-unavailable` | **argent could not see the screen** — `determinacy: indeterminate`, so this is _not_ a failed assertion               | **Environment, not the flow. Re-run.** Check the device/emulator and that the app is attached (`native-devtools-status`). **Do not edit the flow** — a selector "fix" here is a guess against a screen nothing ever read.                                                                                                                                          |
| `scroll-target-not-found`, `scroll-container-not-visible`                                                                                                           | the target never came into view / the anchored container was not visible                                              | Check the scroll container and direction: `direction: up\|down\|left\|right`, and `within: <sel>` to drive a nested scroller. Confirm the target is really inside that scroller.                                                                                                                                                                                   |
| `snapshot-diff`                                                                                                                                                     | pixels differ beyond `maxMismatch`                                                                                    | **Review the diff image** — it is inlined on the snapshot step's own artifact lines, above the summary, not inside the `Failures:` block. Intended change → re-run with `updateBaselines: true` (`--update-baselines` from the CLI), then have the user review and commit `__baselines__/`. Unintended → a real visual regression; report it, don't rebaseline it. |
| `snapshot-baseline-missing`                                                                                                                                         | no baseline exists for this device class                                                                              | Adopt it with `updateBaselines: true` (`--update-baselines` from the CLI), then review and commit the new baseline. Pin the device class (`device` / `platform`) so later runs compare against the same key.                                                                                                                                                       |
| `snapshot-dimension-mismatch`, `snapshot-crop-empty`                                                                                                                | the capture's size changed, or `cropOn` resolved to an empty region — nothing was compared                            | Re-key the baseline for the new device class, or crop on a fixed-size container addressed by `id` (see [`snapshot` cropping](flow-yaml.md)).                                                                                                                                                                                                                       |
| `launch-failed`                                                                                                                                                     | the app never started                                                                                                 | Check the bundle id / package name and that the app is installed on the bound device. Not a selector problem.                                                                                                                                                                                                                                                      |
| `run-cyclic`, `run-depth-exceeded`, `run-fragment-load-failed`                                                                                                      | the `run:` composition graph is broken                                                                                | A flow ultimately runs itself, the chain is too deep, or the named fragment doesn't exist/parse. Fix the composition, not the steps.                                                                                                                                                                                                                               |
| `gesture-geometry-unsatisfiable`, `directive-unsupported`                                                                                                           | the gesture cannot be placed on this screen, or the directive doesn't exist on this target                            | Rework the step: a smaller `pinch`/`rotate`, or — on a Vega TV target, where the touch directives don't apply — `tool: tv-remote` / `tool: keyboard` steps.                                                                                                                                                                                                        |
| `tool-ui-wait-unmet`, `tool-step-failed`, `directive-threw`, `step-kind-unsupported`, `unclassified`                                                                | a raw `tool:` step failed, or the failure resisted classification                                                     | Read `message` and `cause` (the underlying error code) and treat it as that tool's own failure.                                                                                                                                                                                                                                                                    |

## Correct the smallest justified unit

- For one parameter or selector error, edit the YAML and prefer a stable selector — when the failure block ranked `candidates`, paste the winning one's selector YAML in.
