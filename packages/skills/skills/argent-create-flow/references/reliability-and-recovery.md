...
Argent refuses `com.apple.*` bundle ids by policy at every native-devtools read that names one: the flow tree's pinned read, its unpinned fallback to the launched id, and the `native-*` tools. A system app is never the app under test, so `non-injectable` is a refusal, not a failure to load — the instrumentation has been seen both loading and not loading into one, depending on the simulator runtime, and neither is a basis for a selector. `restart-app`, `launch-app`, and `describe` still work on one; it just never gets a flow tree, and no relaunch changes that.
