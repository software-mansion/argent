...
- Vega uses `tool: tv-remote` and raw `tool: keyboard`, which carries `text` or `key` there; `clear: true` is rejected. The runner rejects the touch directives (`tap`, `long-press`, `swipe`, `type`, `scroll-to`, `pinch`, `rotate`) up front, before it resolves a selector. Gate focus and navigation results with `await`.
- **Apple TV and Android TV are not flow targets.** Their ids classify as `ios` and `android` — an Android TV serial is indistinguishable from a phone's, so check `runtimeKind` in `list-devices` — and the runner's touch-directive rejection is gated on Vega alone. It will therefore ACCEPT a `tap:` or `type:` on one instead of rejecting it. On Android TV the gesture reaches the device and does nothing useful at a focus-driven UI; on Apple TV it fails outright, because every touch directive resolves the simulator-server and that refuses a tvOS id. Nothing in the engine stops you; do not author them. `keyboard` still refuses `key` and `clear` on both. Drive those two with the tools directly, per the argent-tv-interact skill.
