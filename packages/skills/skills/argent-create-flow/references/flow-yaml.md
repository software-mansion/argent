...
A **raw `tool: debugger-connect` or `tool: debugger-log-registry` step that succeeded** carries a warning where the answer held a `note`, quoted verbatim. It settles a session that ended holding console logs: it names the log file where that session kept one, and says the entries are gone where it kept none or the file has since been reclaimed. Read it in the report, since both tools spend the record as they read it and no rerun will produce it again. `debugger-log-registry` warns in the same shape for a second reason: its own log file is missing from `~/.argent/tmp`. No read spends that one, and the writer opens its file once, so it returns for the rest of that session however the directory is fixed meanwhile. Only a fresh debugger session clears it, and `debugger-connect` hands back the live one: end it with a scoped `stop-all-simulator-servers` first. No other tool's `note` warns.
