...
On an **Android phone or tablet** the runner reads the field back after it types, because `adb input text` injects the string as one key-event burst that a field re-rendering per keystroke silently drops part of. A read-back that proves the text is not there **fails the step**, and no Enter is pressed; the runner repairs the field first where it can prove which characters are its own, so a step that passes can have deleted and retyped what it typed. A read-back that cannot conclude **passes carrying a warning** — the same for a raw `tool: keyboard` step and a `run-sequence` holding one, which is the form the recorder writes. Read the warning before accepting the step: see [argent-qa-flows](../../argent-qa-flows/SKILL.md) for which causes an edit can resolve.
