...
| -------- | --------------------------------------------------------- | ------------------------------- | ------------------------------------------------------------------------------------------------------ |
| iOS      | projected UIView hierarchy                                | accessibility tree              | `native-full-hierarchy` is raw; nodes and roles differ, and a `launch:` also changes which app is read |
...
**On iOS, a `launch:` step also decides which app the runner reads.** A successful `launch:` pins later runner-tree reads to that app: a pinned read probes only that app, instead of fanning out over every connected one to find the frontmost. It still refuses, with a named reason, when the app has no foreground presence left, when it stops answering after an earlier read got through (usually the suspension iOS applies once a flow leaves an app, though in-app work that blocks the main thread reads the same from outside), when its devtools connection dropped, or when the pinned id is a `com.apple.*` system app. Any raw `tool:` step ends the pin, because its effect on the screen is opaque to the runner, and reads auto-detect the frontmost app again until the next `launch:` re-pins. A tool that cannot change the foreground app still leaves the launched id behind as an unpinned fallback: it decides nothing while auto-detection answers, and takes the read only when auto-detection's probe of every connection times out, the launched id is not a `com.apple.*` system app, the launched app is still connected, and the launched app itself answers a direct probe with a foreground-like state - a fallback that cannot answer leaves the timeout standing, and one that answers with no foreground presence left is refused rather than read. `launch-app`, `restart-app`, `reinstall-app`, `open-url`, and `button` can change the foreground app, so they drop that fallback too - the last three with nothing left behind, while `launch-app` and `restart-app` replace it with one naming the app they just started, still unpinned and subject to the same conditions. Nested `run:` fragments share the run's state, so they inherit both the pin and its clearing. Only iOS tree reads consult the pin.

So on iOS recording and replay can read different apps, not only different projections. Recording has no run state and always auto-detects the frontmost connected app, while a replay read between a `launch:` and the next raw `tool:` step reads the launched app. Foreground-like is not frontmost, so when the launched app and a sibling both qualify the two diverge: recording resolves a uniquely active sibling, or refuses a tie as ambiguous, while replay stays on the launched app.
