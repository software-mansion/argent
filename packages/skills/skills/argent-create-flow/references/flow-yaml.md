...
`type` presses Enter in a second `keyboard` call unless `submit: false`. A polished focus tap plus one text-only `keyboard` call usually needs `submit: false`. `type` has no clear form: it focuses and types, so into a field that already holds a value it splices the new characters into the old one and every run replays that wrong value. Where the field can arrive non-empty, leave the steps raw instead of polishing them into `type:` — `tap:`, `wait: 500`, `tool: keyboard` with `args: { clear: true }`, `tool: keyboard` with `args: { text: ... }`, and a `tool: keyboard` with `args: { key: enter }` in place of the Enter `type` would have sent. Store external values as `{{secret:NAME}}`. The runner uses the first source that defines the name: environment `ARGENT_SECRET_NAME`; project `.argent/secrets.env`; project `.env.local`, then `.env`; then `~/.argent/secrets.env`. The two `secrets.env` files accept the bare `NAME`, but the shared dotenv files expose only `ARGENT_SECRET_`-prefixed keys, so a bare `NAME=…` in `.env` or `.env.local` stays unresolved. The runner redacts every resolved value, so do not use a placeholder for content a report must show.
...
- Vega uses `tool: tv-remote` and raw `tool: keyboard`, which carries `text`, `key` or `clear: true` there — the clear sends 200 backspaces, so it reaches text behind the cursor only. The runner rejects the touch directives (`tap`, `long-press`, `swipe`, `type`, `scroll-to`, `pinch`, `rotate`) up front, before it resolves a selector. Gate focus and navigation results with `await`.
- **Apple TV and Android TV are not flow targets.** Their ids classify as `ios` and `android` — an Android TV serial is indistinguishable from a phone's, so check `runtimeKind` in `list-devices` — and the runner's touch-directive rejection is gated on Vega alone. It will therefore ACCEPT a `tap:` or `type:` on one instead of rejecting it. On Android TV the gesture reaches the device and does nothing useful at a focus-driven UI; on Apple TV it fails outright, because every touch directive resolves the simulator-server and that refuses a tvOS id. Nothing in the engine stops you; do not author them. `keyboard` still refuses `key` on both, and serves `text` and `clear`. Drive those two targets with the tools directly, per the argent-tv-interact skill.
