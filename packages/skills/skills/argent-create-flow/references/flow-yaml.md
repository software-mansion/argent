...
In a `scroll-to` map, put the selector under `target:`. The map supports `up`, `down`, `left`, and `right` directions. The default is `down`; set it explicitly to reach a target above the viewport or along a horizontal carousel. If the target is already visible and clear of the screen edge, the step dispatches nothing. A visible target flush against the screen edge it entered from (`down`/`right` on touch devices) is nudged slightly further in - same direction, best-effort, bounded - so the following `tap` or `snapshot` is not under the home indicator, gesture-nav bar, or a floating tab bar. The viewport can therefore move even when the target was already visible, and the amount is not proportional to how flush the landing was: any shortfall the runner acts on costs a whole minimum swipe (about a twentieth of the screen), because a shorter gesture would register as a tap. An uncropped snapshot baseline taken right after such a landing may therefore need one re-record, and a landing that sits near that threshold can flip between the two shifts across runs - use `cropOn` for the target instead. The nudge never runs on Chromium (nothing overlays a browser viewport - `scroll-to` itself works there) or at `up`/`left` landings (a continuing drag at the top edge would be pull-to-refresh), and it is skipped when the UI tree shows no scroll container owning the target or when `within` names a region that is not itself a scroller - though a pinned control overlapping a scroller's edge can still cost one bounded gesture. The search itself stops as soon as a scroll round leaves the scrolled container's own content unchanged: a target already on screen is accepted where it sits, and a missing one fails as reaching the end of the scroll rather than exhausting the step's attempts. A spinner or clock ticking outside that container does not hold the search open, and on iOS and Android the container counts even with no id of its own - on Chromium an id-less overflow scroller is absent from the tree, so page-level animation can still run the step to its attempt limit. `tap`, `type`, and `long-press` do not auto-scroll. Add `scroll-to` when the target can be off-screen. Use `within` for a nested scroller.
