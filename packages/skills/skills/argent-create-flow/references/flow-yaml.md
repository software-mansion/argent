...
Argent replaces each value a `{{secret:NAME}}` resolved to with that placeholder in three places only: a failed step's message and stack, and the step's log. It hides no other value:
