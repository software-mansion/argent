...
**`describe` reports focus on Chromium only.** iOS and Android leave it unset — it is a Vega/D-pad signal there — so `describe` gives you no pre-typing focus check on either. On iOS the value check afterwards is what proves the keys landed; on Android the `keyboard` call reads the focused field back itself when the android-devtools helper is available, and its note then says when nothing editable held focus — but on a device where that helper cannot install it types without reading at all, so keep the value check there too. On Chromium, read `focused` before recording `keyboard`.
...
A typing step raises a warning of its own shape: on an Android phone or tablet the read-back could not confirm the text, or it repaired the field to get it in. Read which one the note is. A repair means the app saw the text typed twice, and where it deleted first, the field itself was changed beyond the text the step types.
