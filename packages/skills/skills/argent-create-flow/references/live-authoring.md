...
A [raw `tool: debugger-connect` or `tool: debugger-log-registry` step](flow-yaml.md#directives) raises a third shape: the answer held a `note`, which the warning quotes. It settles a session that ended holding console logs — naming its log file where one was kept, or saying the entries are gone. Both tools spend the record as they read it, so a rerun will not produce it again: read it in this report. `debugger-log-registry` warns in the same shape for a second reason: its own log file is missing from `~/.argent/tmp`. No read spends that one, and the writer opens its file once, so it returns for the rest of that session however the directory is fixed meanwhile. A fresh debugger session is what clears it.
