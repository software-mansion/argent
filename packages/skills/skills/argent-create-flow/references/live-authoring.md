...
A typing step raises a warning of its own shape: on an Android phone or tablet the read-back could not confirm the text, or it repaired the field to get it in. Read which one the note is, and treat a repair as a field that was changed beyond the text the step types.
