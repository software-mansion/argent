...
Try the `hint:` line of a failed step first, when the step has one. Many failures carry none: `assert` and `await` with `exists`, `visible` or `hidden`, `launch:`, `script:`, `run:`, and every `tool:` step, `await-ui-element` included. Read the reason of such a step. `expected:` and `actual:` lines hold the values a text or snapshot check compared.
