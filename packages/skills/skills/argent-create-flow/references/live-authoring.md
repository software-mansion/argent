...
If characters are lost, restore the field with direct calls. Do not record a duplicate typing step. Polish the valid pair into `type:`. Its replay focus wait reads the runner's own tree, which does report focus on iOS, Android, and Chromium, and refuses to type until the tapped field reports keyboard focus — an unconfirmed poll retries once and then fails the step with nothing typed (a tree that cannot be read during the wait scores an environment error instead), so the committed-value check remains your proof the value is right. Store credentials as `{{secret:NAME}}`. Never record a literal credential.
...
Manual rescue invalidates the pass. An `errored` step was never evaluated: an `idle` wait whose tree source could not be read, a step that threw, an unresolvable `run:` target, or a `launch:` that did not start the app — plus a `type:` whose field never took focus while the UI tree was unreadable, which scores an environment error rather than an app verdict. Read the reason — most name the environment, but a failed `launch:` is a verdict about the app. Unconfirmed focus IS in the failure class: the replay focus wait fails a `type:` step whose focus was never confirmed after one retry tap, with nothing typed, so a green `type:` means the field held keyboard focus.
