...
When the user requests a local `.mjs` or `.sh` script, call `flow-add-script` at the point where it must run. Read [Flow YAML: Local scripts](flow-yaml.md#local-scripts) first. If the call fails, its own message says whether the script ran: `was NOT run and nothing was recorded` (the pre-run read of the flow file, and the refusal of the `env` argument) and `did not run` leave nothing behind; every other wording (`failed`, `may have run`, `passed, but the step was not recorded`) leaves changes to check before you retry.
...
4. Treat `flow-start-recording` as destructive. It always truncates the named YAML, including a finished or committed flow. The top-level `env:` is the one part it keeps, so a committed flow's script defaults still apply to the steps you record. `restarted` reports only a displaced live take.
