...
Read a failed step in this order. The reason is the fact. A `hint:` line under the step is the first thing to try. `expected:` and `actual:` lines hold the values a text or snapshot check compared; `actual` is cut at 300 characters. The hint `argent could not read the screen, so this is not a verdict on the app …` marks an `await:`, `assert:` or `when:` check that could not read the UI tree, and an errored `idle` (below) is the same case: rerun before editing the flow. `argent flow run --json` marks both `indeterminate: true`.
