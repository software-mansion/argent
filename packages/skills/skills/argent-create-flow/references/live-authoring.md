...
4. Treat `flow-start-recording` as destructive. It always truncates the named YAML's steps, including a finished or committed flow. An existing `requires:` block is the one thing carried across, and only when the flow file sits on the tool server's host and parses — against a remote tool server, or over a file that does not parse, it is lost too and the message says so. `restarted` reports only a displaced live take.
