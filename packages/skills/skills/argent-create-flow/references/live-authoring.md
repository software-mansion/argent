...
Some of these refusals ran first and some ran nothing: a prerequisite notice executes no step, and a batch sends nothing when it was rejected before its first step, or when the only steps it completed were `await-ui-element` waits, which poll the tree and act on nothing. Do not infer which from the case. Read `message`: it warns that the device may have moved when a step was reached, and stays silent when none was. On that warning, treat the device as possibly moved off your recorded prefix's end state — check with `describe` or `screenshot` and restore it by hand. Relaunch the app only while your prefix is the recorded `launch` step alone. After any later step, a relaunch lands on the start screen, not where your prefix ended.
