...
On Android, add `metroPort` when the app is an expo dev build and its Metro is not on port 8081. A `launch:` step then opens that server if it lands on the dev-client chooser. See [the recovery](reliability-and-recovery.md#expo-dev-client-chooser).
