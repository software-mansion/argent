...
One uninterrupted full pass completes a normal flow. `argent-qa-flows` requires two consecutive passes of unchanged YAML. For CI, use `argent flow run <name> [--platform ...]`; it exits non-zero on failure. When `argent flow run <dir>` has failures, it prints a `Failed flows` section above its summary line: each failed flow with its failing step (or, with no report, its outcome), the reason, and a `re-run:` command for that flow alone.
