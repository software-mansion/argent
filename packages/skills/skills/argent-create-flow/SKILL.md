...
A map selector may also carry **`within: <selector>`** — a container scope: match only elements whose on-screen frame sits **inside** the frame of an element matching the nested selector. `tap: { text: Delete, within: { id: profile-card } }` taps the Delete button in the profile card even when other cards show identical Delete buttons; `tap: { text: "Pin feed", within: { text: "For You" } }` picks one card's button out of a whole list of them. Containment is **visual (frame-based), not tree ancestry** — flow trees are flattened, and "inside the card" means what the screen shows — the same frame-based reading of "within" that `scroll-to`'s container anchor uses. So scope to a container with a **tight frame** (a row, card, dialog, toast): a full-screen wrapper contains everything and scopes nothing. The nested slot takes every selector form — a bare string keeps the loose identifier-first fallback (`within: profile-card`), the map form stays strict, the regex matcher works (`within: { text: { matches: '^Card \d+$' } }`) — and scopes chain outward: `{ text: Save, within: { id: cards, within: Settings } }` reads "Save inside cards inside Settings", each container's frame inside the next. Three rules: the container must be a **distinct element** (an element never scopes itself, so `{ id: card, within: { id: card } }` needs two nested elements), the synthetic screen root never counts as a container, and `within` only narrows _where_ to look — the selector still needs its own `text`/`id`/`role` naming _what_ to find there. It works in every selector slot, conditions included: `assert: { hidden: { text: Saved, within: { id: toast-area } } }` holds when nothing matching "Saved" is inside the toast area — matches elsewhere on screen don't count, and a missing container satisfies `hidden` (and fails `visible`/`exists`). `within` is flow-YAML only; the raw `await-ui-element` tool's selector does not accept it. Prefer a unique `id` on the target itself when one exists — reach for `within` when the target has no unique locator of its own (repeated row actions, per-card buttons, list cells).
