...
5. **Polish**: **read the saved `.yaml` file** and convert the raw `tool:` steps that have a cleaner directive form (the recorder leaves these as tools):
   - `tool: keyboard` typing into a field → `type: { into: "<field>", text: "…" }`, folding in the `tap` that focused the field. A submit is recorded as its **own** `tool: keyboard { key: enter }` step (the tool takes text or key, never both) — drop it, since `type` already presses Enter, or keep it and set `submit: false`. Leaving both submits twice.
