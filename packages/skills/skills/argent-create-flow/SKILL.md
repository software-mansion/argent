...
| `type`       | `- type: { into: email, text: "a@b.com" }`, `- type: { into: email, text: "a@b.com", clear: true }`, `- type: { into: search, clear: true }`                         | focus a field, type, then press Enter to submit + dismiss the keyboard; `clear: true` empties the field first — without it the old value stays and the new text lands at the caret the tap set (spliced _into_ it on Android and Chromium, appended on iOS). A `clear` with no `text` only empties the field, and sends no Enter unless you add `submit: true` |
...
`type` presses Enter after typing to commit the value and dismiss the keyboard, so it can't cover later targets. For a chained form whose fields feed one explicit submit — e.g. email then password then a `tap: "Log in"` — set `submit: false` on the intermediate fields so a premature Enter doesn't fire the form early: `type: { into: password, text: "hunter2", submit: false }`. A `clear`-only step (no `text`) defaults to `submit: false` instead, since Enter into a field it just emptied would fire an empty search or form.

`type` never **replaces**. The old value survives, and the new text lands wherever the focusing tap put the caret — which is not always the end: the tap goes to the middle of the field, so on Android and Chromium a value long enough to reach that point gets the new text spliced _into_ it (a field holding forty `A`s, typed with `ZZZ`, ends up `AAA…ZZZ…AAA` — measured on both), while on iOS the caret jumps to the end and the text really does append. Set `clear: true` whenever the field may already hold a value — a remembered login, a restored draft, a fragment re-run without a `launch`, a retry after a failed step: `type: { into: email, text: "new@example.com", clear: true }`. Without it the flow fails later at an `assert`, pointing at the wrong place.

`clear` works on iOS, Android and Chromium; it is rejected — loudly, before anything is typed — on TV targets: an Apple TV's backend has no modifier-chord primitive at all, and routing one through Android TV's is unverified, so neither family gets a half-supported `clear`. (On Vega the whole `type` directive is rejected, `clear` or not — see _TV targets_.) A clear-only step sends no Enter; add `submit: true` when the empty value itself has to be committed, e.g. to reset a live-filtered list.

The step will not clear on bad evidence: it dispatches the clear only when the tree reports focus INSIDE the field you named, and **fails** when focus is reported anywhere else — including on a node that merely _covers_ your target (a shadow host, a focused WebView, a focus trap), which is not evidence the keys will go where you meant. One residual: when the tree reports focus on **nothing at all** the clear does go through, because that is also what an iOS build without `firstResponder` looks like, and what a field the app blurred — or scrolled off screen — looks like. A clear with nothing focused loses no data; a clear on the wrong field does. It does not read the field back afterwards, though. If a flow must prove the new value took, follow it with an `assert` on the field's text — but check `references/asserting-field-values.md` first: a text field's contents reach the flow tree unevenly (never on iOS, never for a password, and on Chromium only as the accessible name), so the obvious assert hard-fails on a perfectly good clear. Where the contents are invisible, assert the consequence instead.
...
A Vega (Fire TV) device is remote-driven — there is no touch input, so the touch directives (`tap`, `long-press`, `type`, `scroll-to`, `pinch`, `rotate`) fail on it with guidance. Drive focus with `tool: tv-remote` steps and type with `tool: keyboard` instead (its `clear` is rejected there — Vega cannot send the select-all chord); everything else (`launch`, `await`, `assert`, `wait`, `snapshot`, `echo`, `run`, selectors) works unchanged — the tree comes from the on-device automation toolkit, which attaches at app launch (the `launch` step waits for it, so a leading `launch` also guarantees selectors resolve).
...
5. **Polish**: **read the saved `.yaml` file** and convert the raw `tool:` steps that have a cleaner directive form (the recorder leaves these as tools):
   - `tool: keyboard` typing into a field → `type: { into: "<field>", text: "…" }`, folding in the `tap` that focused the field. **Carry `clear` across**: a recorded `tool: keyboard` step with `args: { clear: true }` becomes `type: { into: "<field>", text: "…", clear: true }`, and one with `clear` but no `text` becomes `type: { into: "<field>", clear: true }`. Dropping it turns a replace back into typing on top of the old value, which is a data bug the cleaned flow will only reveal at a later `assert`. **Carry the recorded `key: "enter"` too**, as `submit: true` — a clear-only `type` sends no Enter by default, so a recorded `{ clear: true, key: "enter" }` silently loses its submit otherwise. Conversely a recorded `keyboard` step with `text` but _no_ `key` needs `submit: false`, since `type` adds an Enter the recording never sent.
