...
| `repeat`     | `- repeat: 3` + `steps: [...]`, or `- repeat: { until: { hidden: "Clear notification" }, max: 15 }` + `steps: [...]`                                                 | run the same step block N times, or until a condition holds (hitting `max` fails); **not** retry — a failure inside an iteration hard-stops                                                                                   |
