...
description: Create, record, edit, replay, or repair reusable Argent flow YAML files. Use for repeatable device paths, replay, profiling or A/B setup, and the authoring engine behind argent-qa-flows. Also use before repeating three or more interactions. For one-off UI checks, acceptance-driven regression tests, or screen video, use argent-test-ui-flow, argent-qa-flows, or argent-screen-recording respectively.
---
