...
| `snapshot-diff`                                                                                                                                                     | pixels differ beyond `maxMismatch`                                                                                    | **Review the diff image** in the failure block. Intended change → re-run with `--update-baselines`, then have the user review and commit `__baselines__/`. Unintended → a real visual regression; report it, don't rebaseline it.       |
| `snapshot-baseline-missing`                                                                                                                                         | no baseline exists for this device class                                                                              | Adopt it with `--update-baselines`, then review and commit the new baseline. Pin the device class (`--device` / `--platform`) so later runs compare against the same key.                                                               |
