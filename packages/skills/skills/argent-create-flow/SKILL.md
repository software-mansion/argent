...
It is **not** a screen check: a dropped tap leaves the source screen perfectly idle. Put it **after** the element `await:` that names the destination, never instead of one. Do not sprinkle it after every step — each one costs a settle; add one where a step actually proved flaky. If its captures never produced a comparable pair, it still passes on the tree alone but reports a warning saying so, because it then only proved the hierarchy held still.
