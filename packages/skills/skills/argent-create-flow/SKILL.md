...
A flow is a sequence of steps saved to a `.yaml` file in the `.argent/flows/` directory. Each recorded step is **executed live** as you add it, so you verify it works before it becomes part of the flow — with two caveats worth knowing up front: an `await-ui-element` whose condition never held still gets recorded, and a check that passes live can still fail once polished into an `await:`/`assert:` directive, because those resolve against a different tree. The recorder tells you about both in the step's `message`. Replay a finished flow with `flow-execute`, or — for an e2e flow — headlessly with `argent flow run checkout` (a saved flow's name) or `argent flow run path/to/checkout.yaml`.
...
Selectors resolve against the **full native hierarchy** (iOS: the UIView tree; Android: the complete accessibility hierarchy including not-important views), with complete `testID`/`resource-id` coverage. So an `id` selector works even when `describe` collapses or omits the element — don't fall back to coordinate taps just because a testID isn't visible in `describe` output. But the flow tree is **not a superset** of what `describe` or the raw `await-ui-element` tool see: it keeps only views a selector can address _and_ that are actually on screen, so it drops what those tools still report — hidden, transparent and scroll-clipped views on iOS, system chrome on Android, non-addressable and off-viewport nodes on Chromium — and an iOS AX leaf whose merged label aggregates a whole `accessible` container matches no single view inside it. **Neither tree contains the other**, which is why recording an `await-ui-element` step re-probes the condition against the flow tree and warns you when the two disagree (see Recording). And when several elements match — including wrappers whose native text aggregates descendant content — the action directives (`tap`, `type`, `scroll-to`) pick the **most specific** match: an exact text/identifier match beats a substring hit (for a regex matcher, a pattern consuming the element's whole text counts as exact), then the smallest frame wins. (A universal `any: true` selector has no field to be exact about, so its matches rank by reading order instead — the first element in the scope, which is the element a condition reads too. Where two matches share a top-left corner, an action breaks the tie toward the smaller, more specific one and a condition does not, so those two can name different elements.)
...
For a custom poll interval or bundleId, drop to an explicit `- tool: await-ui-element` step — but the raw tool polls the trimmed `describe` tree, so a testID it reports as not found can still resolve fine as an `await:` directive, and a check it passes can fail once converted (see Selectors). Prefer the directive.
...
Track what has been recorded from each call's return: `flow-add-step` gives you `recorded` — a one-line summary of the step it just appended, in the flow file's own spelling — and `stepCount`, the running total; `flow-add-echo` gives you `stepCount`. The full flow file is deliberately **not** returned per step (it grows with every call): `flow-start-recording` and `flow-finish-recording` return it as `flowFile`, and `savedTo` on every one of them names where the YAML landed. Read `recorded` — a step is not always stored as the tool call you made (a coordinate `gesture-tap` becomes a `tap: { selector }`, a `restart-app` becomes a `launch`), and that line is where the rewrite shows up. A **`savedTo` that comes back `null`** means the file could not be written on your side (a rejected path, or a failed write); the step is still in the recording, so keep going — the next step rewrites the whole file, and `flow-finish-recording` returns the YAML as `flowFile` regardless. Rules:

- **Every step runs live.** You see the real tool result (including screenshots) — verify the step worked before continuing. **Only steps that returned successfully are recorded**: a call that _fails_ writes nothing to the flow file; fix the issue and try again. The exception is `await-ui-element`, which reports an unmet condition by RETURNING `{ success: false }` rather than failing — so an unmet wait is recorded, and at replay it stops the run at that step. The recorder says so in `message`; when it does, delete the step from the `.yaml` or re-record it once the condition can hold.
...
- **Mistakes can be edited out.** Edit the `.yaml` file directly to remove or reorder steps. Mid-recording, this holds only when the tool-server shares your filesystem (the usual local case): the recorder re-reads the file before each append, so your edit is kept — provided it still parses, since the next step re-validates the whole file and fails on a botched one instead of quietly keeping it. Against a **remote client** the in-memory copy is authoritative and the next append overwrites the file, so edit after `flow-finish-recording`. Editing after the recording is finished is always safe.
...
5. **Polish**: **read the saved `.yaml` file** and convert the raw `tool:` steps that have a cleaner directive form (the recorder leaves these as tools):
   ...
   - `tool: await-ui-element` gating a transition → `await: { visible: "…" }` / `{ hidden: … }` / `{ text: { in: …, equals: … } }`, carrying a custom `timeoutMs` over as a `timeout` sibling key. Converting also moves the wait from the trimmed `describe` tree to the flow's full-hierarchy tree, and neither contains the other (see Selectors) — so check `message` on the recorded step: if the recorder warned that the condition does NOT hold against the runner's tree, converting is exactly what would break it, and the step should stay raw or be re-recorded. Keep the raw `tool: await-ui-element` step only when it sets a custom `pollIntervalMs`/`bundleId` the directive can't express.
...
3. Call `describe` or `debugger-component-tree` to get the current element tree. Remember the two trees each hold what the other drops — a testID missing from `describe`'s output can still resolve as a selector, and an element `describe` shows can be absent from the flow tree (see Selectors).
