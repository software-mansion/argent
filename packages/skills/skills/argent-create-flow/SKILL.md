...
#### Scopes — the CSS combinators, read off frames

A map selector may also carry a **scope**: a nested selector naming another element the match must sit in a given spatial relation to. The three are the geometric readings of the CSS combinators, and they combine and nest freely:
...
And `any: true` is the CSS `*` universal selector — no locator of its own, so it is accepted only **with** a scope: `tap: { any: true, next: { text: Airplane Mode } }` taps whatever sits right after the Airplane Mode label.
