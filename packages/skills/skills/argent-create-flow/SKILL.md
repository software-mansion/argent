...
`scroll-to` takes an optional `direction` (`up` | `down` | `left` | `right`, default `down` — so the common case is just `- scroll-to: <selector>`) and optionally a `within: <selector>` that anchors the scroll inside a specific container — required to drive a **nested** scroller (e.g. a horizontal carousel inside a vertical list), since the device can't be asked which container to scroll. This step-level `within` (a sibling of `target`) anchors the _gesture_, and it is the **only** scope key the step body takes — `after:`/`next:`/`any:` beside `target` are rejected. It is distinct from a selector's scopes (see Selectors), which `target` may itself carry — `scroll-to: { target: { text: Delete, within: { id: cards } }, within: { id: settings-list } }` scrolls the settings list until the Delete button _inside the cards container_ is visible. It scrolls in bounded momentum-free increments, re-checks after each, and stops if a scroll reveals nothing new (end of the container). Once the target is visible, a landing that sits nearly flush against a screen edge gets nudged a bit further in (same direction, best-effort) so the following `tap`/`snapshot` isn't under the home indicator, gesture-nav bar, or a floating tab bar — skipped when the UI tree shows no scroll container to measure against — the target's own when the step names no `within`, otherwise the named container itself (so a pinned bottom bar, a static screen, or a `within` naming a card or a text pane never gets nudged, even when that card sits inside a scrollable page) — when that container is itself inset from the screen edge, when the target has no room to move, when the landing is against a start edge (`up`/`left`, where the continuing drag would be pull-to-refresh, so those stay flush), or once the container stops scrolling (last element). It never runs on a browser target: nothing overlays a page's viewport, so there is no chrome to clear. `tap`/`type` do **not** scroll — add a `scroll-to` before any target that may be off-screen. It's a no-op when the target is already visible and clear of the screen edge, so a defensive `scroll-to` costs little on replay and keeps the flow working on smaller screens.
