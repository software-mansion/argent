...
**`after` / `next`** — reading order is row-band aware: an element **follows** the anchor when it starts below the anchor's bottom edge, _or_ shares its row band and sits entirely to its right. That is what makes `{ role: Switch, next: { text: Wi-Fi } }` resolve the Wi-Fi row's own switch even though the taller switch's frame starts a couple of pixels _higher_ than the label's. `next` keeps only the nearest follower — a match in the anchor's own row beats anything on the rows below, leftmost first — while `after` keeps them all, so `assert: { hidden: { role: Button, after: { text: Danger zone } } }` holds when nothing button-like appears past that heading. Both union over anchors exactly as CSS does: with three rows on screen, `{ role: Switch, next: { role: AXStaticText } }` yields all three switches, one per label. Note that "follows" is a partial order, not a total one — an element sharing a tall anchor's row band but sitting to its left follows neither it nor what follows it — so `after` chains do not compose the way CSS `~` chains do; nest them only when each link is a container-sized step. An element sitting _inside_ the anchor does not follow **that** anchor — containment is not reading order — so scope by `within` for that.

**Use the map form for a sibling anchor.** A bare-string scope (`next: wifi-row`) keeps the usual loose identifier-first fallback, but that fallback can only rescue itself for `within`: a wrong container matches nothing, so the text pass gets its turn. A wrong _anchor_ matches plenty — everything after it — so the identifier pass "succeeds" and the text pass never runs. If some unrelated element carries `testID="Wi-Fi"`, `tap: { role: Switch, next: Wi-Fi }` silently taps whatever follows _that_ and reports a pass. Spell sibling anchors `next: { text: Wi-Fi }` or `next: { id: wifi-row }`.
