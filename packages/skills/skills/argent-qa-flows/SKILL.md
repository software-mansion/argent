...
description: Create repeatable QA regression E2E tests as Argent flows from test cases, tickets, or acceptance criteria. Use to generate or preserve automated regression scenarios with deterministic setup, stable targets, executable structural or visual evidence, and two consecutive full passes. For one-off UI checks or replayable paths without acceptance criteria, use argent-test-ui-flow or argent-create-flow. Saved QA regression flows are unsupported on TV/Vega. Use argent-tv-interact there and report the limitation.
---
...
For TV/Vega, stop and use `argent-tv-interact` for one-off interaction. Report that Argent cannot produce an equivalent saved QA regression artifact.
...
5. Targets satisfy the stable-selector and coordinate-fallback rules. QA keeps coordinates only for genuinely unlabeled targets.
...
2. Build a navigation table with one row per screen change.
...
A missing identity check must be recorded live. A missing `idle` check uses the documented polish insertion. Re-record any missing action or other structural check.
...
3. Before mobile pass 1, recycle Argent services for this flow's device. Scope `stop-all-simulator-servers` to `devices: [<device>]`. Use the MCP call for `flow-execute`, or `argent run stop-all-simulator-servers` from the standalone runner's install. The reset must not change app or account data. For Chromium, let the runner boot the declared app and omit `device`.
4. Run from the flow's launch and setup without baseline-update mode. Count a pass only when `ok: true` and every acceptance check executed. A false `when:` can skip optional setup only. An errored step is an environment result and does not advance the streak.
5. Resolve every passing-step warning before completion. For `idle`, inspect the screen and verify that surrounding acceptance checks use stable elements.
