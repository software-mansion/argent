...
Never put acceptance evidence inside `when:` or inside an [`until` drain body](../argent-create-flow/references/flow-yaml.md#draining-to-a-condition). Use `when:` only for optional setup that reconverges to the required path.
