...
5. Resolve every passing-step warning before completion. Also resolve recorded-wait warnings from `flow-finish-recording`. Follow [Live waits and checks](../argent-create-flow/references/live-authoring.md#live-waits-and-checks). For runner warnings, `await: { idle: true }` raises [six different warnings](../argent-create-flow/references/flow-yaml.md#idle-readiness), so read which one it is first. Two say the screen was moving. One says the wait ran out mid-hold and needs a larger `timeout:`. One says the tree stayed empty. One — **settled on the UI tree alone** — says the hierarchy did hold still and only the screenshot pairs were missing, so inspect the capture path rather than the app's rendering. One says the step ended with no evidence either way. Inspect the screen, disclose the cause, and verify that surrounding acceptance checks use stable elements rather than stillness. A [selector-less gesture](../argent-create-flow/references/flow-yaml.md#directives) — a coordinate `tap`/`long-press`/`swipe`, or a `pinch`/`rotate` with no `on:` — warns in a different shape: a tree-source outage left it unsettled, so it dispatched blind and the green says only that the gesture was sent. Restore the tree source, usually by relaunching the app so the instrumentation loads, and rerun. Accepting that warning needs an app that serves no tree, which cannot satisfy this contract anyway. An Android typing step warns in a third shape, whether it is written as `type:`, as a raw `tool: keyboard` step or as a `run-sequence` holding one: the `keyboard` tool read the field back, and either the check could not conclude or a repair ran to get the text in. Read which one the note is. A note that reports a repair says what it retyped, and the note for a repair that worked gives the number of characters it deleted first, so the app saw the text typed twice and, where the repair deleted first, the field was changed beyond the text the step types - confirm the field holds what the contract expects. A note that reports an unverified type names its cause, and the cause decides what to do. Three are properties of the app or the device rather than of the run: the field masks its input, the android-devtools helper could not be reached, or focus sits on an editor the read-back does not recognise. Those repeat on every pass and no edit resolves them, so disclose the cause and report the step as typed but unverified. The helper is the one to read twice: it is permanent where the device blocks `adb install -t` and worth one rerun where it failed to start that time, which the note says. Every other cause belongs to the run. No editable field held focus when the typing started, or focus moved or was lost while the text was typed: read the screen at that step and give the step a field that holds still, the same way a wait warning is resolved - and note that the first of those says the characters may have gone nowhere. The reading fits a success and a failure equally: read the field and decide from what it holds. The read failed, came back empty, or was truncated: the helper or the screen is the cause, not the step, so rerun it and inspect the helper if it repeats. More than one window reported an editable field with focus, which is a dialog over the app: nothing in the read says which one takes typing, so read the screen at that step and either dismiss the dialog or aim the step at the field it covers.
