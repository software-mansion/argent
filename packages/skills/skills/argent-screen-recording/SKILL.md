...
- `screen-recording-start` — start capturing the screen of a booted device to a video file. Frames come from the same simulator-server backend that `screenshot` and the interaction tools already use, and end up as an h264 mp4 (constant 30 fps, device-native resolution).
...
5. Call `screen-recording-stop` with the same `udid`. It returns `{ video, durationMs, wallClockMs?, trimmedMs?, warning? }`; `video` is an artifact whose resolved path points at the durably-saved file (see below). The video is final when stop returns. On builds where simulator-server does the recording, stop asks it to mux the mp4 and then copies the file out, so stop can take longer for long recordings; the host-ffmpeg fallback has encoded every frame during capture and only closes and flushes the container at stop, so it usually returns in well under a second (it waits on ffmpeg up to ~20s before forcing it).
...
7. **If `screen-recording-stop` fails, read the error before giving up:**
   - A timeout, or "its stop request failed before simulator-server confirmed the hand-over", means the recording may still be running inside simulator-server. The reminder note stays, pointing you back — **retry the stop**; that is what the recovery is for.
   - "could not be copied out of simulator-server" means the recording finished but writing it to its destination failed (a full disk, an unwritable recordings directory). The session is already reset, so a stop retry will not re-copy it — do **not** retry the stop. The finished mp4 is still at the server-side path the error names, but **only until that simulator-server exits**: if you can reach that host, copy it from there yourself; otherwise clear what blocked the write and record again.
   - Any other failure (connection refused, the server rejected the command) has already reset the device to startable, so start a fresh recording rather than retrying the stop.
...
- **ffmpeg is needed unless simulator-server encodes for itself**: builds that expose a recording endpoint record and mux the video inside the server, and nothing extra is installed. Every other build — and every simulator-server predating the endpoint — is encoded host-side instead, and there `screen-recording-start` fails up front with an install hint if `ffmpeg` is missing, resolving it from `PATH` plus the usual Homebrew prefixes. Which one you get is not something to predict from the platform: provide `ffmpeg` (install it, or ask the user to) only when a start actually asks for it.
