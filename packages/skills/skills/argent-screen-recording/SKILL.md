...
- **What can be recorded**: anything simulator-server drives — iOS simulators, connected physical iPhones (behind the `physical-ios-devices` flag), Android emulators, and physical Android devices. The only length limit is `timeLimitSeconds` (max 600).
...
- **Unsupported**: tvOS simulators, Chromium apps, Vega/Fire TV, and remote (`remote:`-prefixed) simulators — none of them expose a readable frame stream. For a single still frame use `screenshot`; for a replayable interaction script use `argent-create-flow` instead of a video (flows do not run on a physical iPhone — drive it with `describe` + `gesture-*`).
