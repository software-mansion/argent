...
- **Unsupported**: tvOS simulators, physical iPhones, Chromium apps, Vega/Fire TV, HarmonyOS devices, and remote (`remote:`-prefixed) simulators — none of them expose a readable frame stream. For a single still frame use `screenshot`; for a replayable interaction script use `argent-create-flow` instead of a video.
