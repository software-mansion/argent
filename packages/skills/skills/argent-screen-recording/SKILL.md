...
7. **If `screen-recording-stop` fails, read the error before giving up:**
   ...
   - "could not be copied out of simulator-server" means the video was finalized but the copy failed. The error names the server-side path, and it is retrievable **only until that simulator-server exits** — retry promptly.
