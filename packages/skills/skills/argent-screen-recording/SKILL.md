...
- **What can be recorded**: anything simulator-server drives — iOS simulators, remote (`remote:`) simulators, Android emulators, and physical Android devices. The only length limit is `timeLimitSeconds` (max 600).
...
- **Unsupported**: tvOS simulators, physical iPhones, Chromium apps, and Vega/Fire TV — none of them expose a readable frame stream. For a single still frame use `screenshot`; for a replayable interaction script use `argent-create-flow` instead of a video.
- **Remote (`remote:`) simulators**: recorded on the runner instead of here — simulator-server buffers the H264 frames it already encodes for the stream, and `screen-recording-stop` is what ends that recording and downloads the mp4, so a long capture takes a moment to transfer. Touch markers are drawn on the runner (and are visible in the live stream until the stop turns them off). Trimming and the watermark run as a post-pass over the downloaded file: still stretches collapse completely, with no grace window, and if ffmpeg is missing the stop hands back the untouched recording with a warning instead of failing.
- **ffmpeg is required for local devices**: it is the encoder, so `screen-recording-start` fails up front with an install hint if it is missing (`brew install ffmpeg`). It is resolved from `PATH` plus the usual Homebrew prefixes. On a remote simulator it is optional — only the post-pass uses it.
