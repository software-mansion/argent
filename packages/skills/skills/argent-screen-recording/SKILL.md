...
- **One size per video**: the video keeps the size of its first frame. If the screen changes size mid-recording (for example a foldable simulator that changes panels), the new frames are scaled to fit and centred on black bars, never stretched.
