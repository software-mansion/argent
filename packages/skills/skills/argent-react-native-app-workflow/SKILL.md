...
| Describe screen (a11y tree)  | `describe` tool for normal app screens and in-app modals; if a permission/system overlay is not exposed reliably, `keyboard` with `key: "enter"` may be acceptable only for an obvious default action; otherwise use `screenshot` only to inspect/report the current state |
