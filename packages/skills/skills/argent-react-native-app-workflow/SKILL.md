...
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **JavaScript errors / logs**      | Use `debugger-log-registry` to get a summary and log file path, then `Grep`/`Read` to search. Read its `note` first whatever the `status`: it says what became of a previous session's console history and names the log file when one was kept, so a zero count on a session that replaced a dead one is not evidence the app logged nothing. On `status: "not_connected"` it returns no log file of its own — a path the note names is still readable — so follow its `guidance`. |
