...
- **Visible UI changes**: Use `argent-test-ui-flow` for manual QA. For `screenshot-diff` rules and parameters, follow `argent-device-interact` §7. Use it when stable before/after screenshots add meaningful pixel-visible evidence.
...
| Compare visible UI changes   | `screenshot-diff` tool; follow `argent-device-interact` §7 for baseline/current capture choices                                          |
