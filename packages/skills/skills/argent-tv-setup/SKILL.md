...
argent drives both **Apple TV (tvOS)** and **Android TV (leanback)** with the same focus-driven tools — the standard `describe` / `button` / `keyboard` tools (which detect a TV target and route to the focus backend) plus the TV-specific `tv-set-focus`. A TV target is identified the same way on both platforms: `list-devices` tags it `runtimeKind: "tv"` (non-TV devices are `runtimeKind: "mobile"`). The id you pass differs by platform — an iOS-shaped UDID for Apple TV, an adb serial for Android TV — but you do NOT need to tell the tools which platform; dispatch is automatic from the id.
...
2. **Verify connection** — the focus-driven tools (`describe` / `button` / `keyboard` / `tv-set-focus`) auto-start the tvOS daemons (`tvos-ax-service`, `tvos-hid-daemon`) on first use; no manual server start needed. The first call may take a few seconds while they spawn.

Notes:

- tvOS UDIDs look like `A1B2C3D4-E5F6-7890-ABCD-EF1234567890` (same shape as iOS — only `runtimeKind` distinguishes them).
- Passing an iPhone (`runtimeKind: "mobile"`) UDID to `tv-set-focus` (or a TV remote `button`) fails with a clear "tvOS-only" / "not a TV remote button" error.
...
2. **Identify it** — `list-devices` tags it `runtimeKind: "tv"`, detected via the system feature list (`android.software.leanback` / `android.hardware.type.television`), **not** the serial and **not** `ro.build.characteristics` (which reads `emulator` on TV AVD images). Pass the **serial** to the focus-driven tools.
...
`<HOST_IP>` is your Mac's LAN IP (it changes between networks — read it off the dev-launcher screen, or check Metro). The dev-launcher also lists recently-used servers, which you can focus and `select` with `button`.

**Android TV (leanback emulator)** — `launch-app` brings up the Expo DevLauncher every time. Two ways to load the bundle:

1. Focus the "Recently Opened" server row and `select` it (use `describe` to find it), **or**
...
After the bundle loads, give the RN JS a few seconds to render before the first `describe` (focus only exists once the bundle has run — see `argent-tv-interact`).

## 4. Both platforms

A TV UI is **focus-driven**, not touch-driven. The coordinate/touch gesture tools (`gesture-tap`, `gesture-swipe`, etc.) are not the right tools on a TV target — use `describe` (read focus), `button` (remote presses), `keyboard` (type into the focused field), and `tv-set-focus` (jump focus by label) instead. See `argent-tv-interact` for the full focus-driven interaction workflow (it covers both Apple TV and Android TV).
