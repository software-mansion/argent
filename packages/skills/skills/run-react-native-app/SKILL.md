...
1. **Verify Metro is ready**: you may verify connection by using `debugger-status` tool.
...
- [ ] If simulator not booted: run `boot-simulator` tool with proper UUID. Refer to `simulator-setup` skill.
...
- **Output with PID** → Another process (often Metro instance started by user) is using 8081.

Try to connect to the metro running `debugger-status` tool. If the app running on the port is not a metro server - ask user whether you may kill the process.
...
**Verify Metro is reachable:** curl for `http://localhost:8081` curl; expect Metro’s debug page or JSON.

### 2.3 Reload the App (Ensure New Bundle)

After code or config changes, the app must load the new bundle:

| Method       | How (agent-friendly)                                                                      |
| ------------ | ----------------------------------------------------------------------------------------- |
| Restart tool | Use the `debugger-reload-metro` tool                                                      |
| Restart app  | Kill the app in simulator and run `npx react-native run-ios` again (Metro still running). |

**Agent checklist:**

- [ ] Only one Metro process (no duplicate on 8081)
- [ ] App was started after Metro was ready
- [ ] Whenn needing to reload: refer to 2.3
...
| Situation                                             | Action                                                                                |
| ----------------------------------------------------- | ------------------------------------------------------------------------------------- |
| JS/React only changed                                 | Reload (Metro "R" or Dev menu Reload). No rebuild.                                    |
...
### 3.3 iOS Simulator Control (CLI)

- List devices: `xcrun simctl list` (or `xcrun simctl list --json`)
- Boot: `xcrun simctl boot "iPhone 16"` (name or UUID)
- Shutdown: `xcrun simctl shutdown <UUID>`
...
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **JavaScript errors**             | LogBox (red/yellow screen in app). React Native DevTools: press "j" in Metro terminal or Dev Menu → "Open DevTools"; check Console. |
...
- **JS logs**: Metro terminal output; or DevTools Console.
- **iOS native**: `npx react-native log-ios` or Xcode console when running from Xcode.
...
- [ ] Prefer user’s existing scripts (e.g. `npm run test:e2e`) when present.
- [ ] If unclear: ask whether to use existing workflows or a one-off command.
...
| Kill Metro on 8081          | `lsof -ti :8081 \| xargs kill`         |
...
_For a concise research summary and sources, see [RESEARCH_SUMMARY.md](RESEARCH_SUMMARY.md)._
