...
description: Interact with a physical iPhone or iPad via argent. Use when tapping, swiping, typing, or reading the screen on a physical iOS device (a list-devices iOS entry with kind "device"); the interaction contract differs from simulators. For flag, signing, cable, or trust problems, use argent-ios-device-setup first.
---
