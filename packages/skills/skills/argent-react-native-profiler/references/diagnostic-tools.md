...
Call `debugger-log-registry`. When connected (`status: "connected"`) it returns a summary with entry counts by level, message clusters, and the log file path. Use `Grep`/`Read` on the log file to filter by level or search for specific messages. When the debugger is unreachable it does not fail — it returns `{ status: "not_connected", reason, detail, guidance }` with no log file of its own; follow the `guidance` (do not retry in a loop). Check `note` on either status: when it names a KEPT log file, grepping that path is what to do next. A connected answer carries one whatever its counts say, so a healthy count beside it is no reason to skip it - those counts are the new session's. A note about this answer's OWN `file` - this session's path, not a dead session's - reaches a `connected` answer only: the counts are real, the clusters are a capped and truncated view of them, and the path is not there to grep. Repairing `~/.argent/tmp` does not clear it, since the writer opens its file once and `debugger-connect` hands back the live session; end that session with `stop-all-simulator-servers` scoped to the id you connected WITH, which for a session opened under a `logicalDeviceId` is the only id that reaches it. A note reporting a PREVIOUS session's kept file as reclaimed also says there is no file at a path, and that one reaches either status.
