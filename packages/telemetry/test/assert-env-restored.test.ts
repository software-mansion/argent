...
describe("assert-env-restored", () => {
  it("watches the pair os.homedir() consults, plus the ones the sweep leaves ambient", () => {
    expect(SCOPED_ENV_VARS).toEqual([
      "HOME",
      "USERPROFILE",
      "CI",
  ...
  it("registers an afterAll that reports every watched variable against the environment as it was at module load", async () => {
    ...
    try {
      ...
      // A long value is cut on both halves, so one leaked OTEL_EXPORTER_OTLP_HEADERS cannot bury
      // the rest of the line.
      process.env.OTEL_EXPORTER_OTLP_HEADERS = "/a".repeat(200);
