...
import type { CDPClient } from "../debugger/cdp-client";
import type { JsRuntimeDebuggerApi } from "./js-runtime-debugger";
import type { HermesCpuProfile, DevToolsCommitTree } from "../profiler/src/types/input";
...
export function cacheProfilerData(port: number, snapshot: ProfilerDataSnapshot): void {
  profilerDataCache.set(port, snapshot);
}

export function getCachedProfilerData(port: number): ProfilerDataSnapshot | undefined {
