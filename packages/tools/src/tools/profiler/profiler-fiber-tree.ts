...
Use to trace ancestry when a finding is a library component, or to check for useMemoCache hook (confirms React Compiler is active on a component).`,
  ...
  async execute(services, params) {
    ...
    type FiberEvalResult = { result?: { value?: string }; exceptionDetails?: { text?: string } };
    ...
    if (typeof parsed === "object" && parsed !== null && "error" in parsed) {
      ...
      throw new Error(
        HOOK_NOT_PRESENT_ERRORS.has(errorMsg) ? HOOK_MISSING_MESSAGE : `Fiber tree error: ${errorMsg}`
      );
    }
