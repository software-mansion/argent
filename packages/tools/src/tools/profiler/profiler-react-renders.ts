...
type ParsedRenders =
  | Record<string, { renderCount: number; totalActualDuration: number; selfBaseDuration: number }>
  | { error: string };
...
Returns a markdown table of the top re-rendering components. No profiling session required — works on a live connected app.`,
  ...
  async execute(services, params) {
    ...
    type EvalResult = { result?: { value?: string }; exceptionDetails?: { text?: string } };
    ...
    if (errorStr !== null) {
      throw new Error(HOOK_NOT_PRESENT_ERRORS.has(errorStr) ? HOOK_MISSING_MESSAGE : `React hook error: ${errorStr}`);
    }
