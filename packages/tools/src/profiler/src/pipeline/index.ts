import type { RawProfilingInput } from '../types/input.js';
import type { PipelineOutput } from '../types/pipeline.js';
...
export async function runPipeline(input: RawProfilingInput): Promise<PipelineOutput> {
  ...
  const hotCommitSummaries = buildHotCommitSummaries(preprocessed, hotCommitIndices);
  ...
  if (reduceOutput.anyRuntimeCompilerDetected && !reduceOutput.sessionContext.reactCompilerEnabled) {
    reduceOutput.sessionContext = { ...reduceOutput.sessionContext, reactCompilerEnabled: true };
  }
