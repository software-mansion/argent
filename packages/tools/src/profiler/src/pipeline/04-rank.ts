 ...
 */
import type { TagOutput } from '../types/pipeline.js';
import type { ComponentFinding, ReRenderReason } from '../types/output.js';

const ACTIONABLE_REASONS = new Set<ReRenderReason>(['parent', 'props', 'hooks', 'context', 'state']);
...
export function rank(input: TagOutput): ComponentFinding[] {
  ...
  const findings: ComponentFinding[] = topCandidates.map(({ name, comp }) => {
    const topChangedHookNames: string[] = comp.hookTypeNames !== undefined
