...
function loadTreeSitter(): { ParserClass: ParserCtor; ts: unknown; tsx: unknown } | null {
 ...
 * Returns empty index if tree-sitter is not available.
 * @deprecated Use buildAstIndexWithDiagnostics for richer output.
 */
export async function buildAstIndex(projectRoot: string): Promise<ComponentIndex> {
  const result = await buildAstIndexWithDiagnostics(projectRoot);
  return result.index;
}
