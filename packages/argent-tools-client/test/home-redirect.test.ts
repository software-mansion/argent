...
describe("redirectHomeTo", () => {
  it("is backed by a setup file that fails any suite leaving the redirect in place", async () => {
    ...
    expect(config.default.test?.setupFiles).toContain("test/setup/assert-home-restored.ts");
  });
