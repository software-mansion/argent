...
export const axServiceBlueprint: ServiceBlueprint<AXServiceApi, DeviceInfo> = {
  ...
  async factory(_deps, _payload, options) {
    ...
    if (device.kind === "device") {
      ...
      // Physical iPhones are driven over CoreDevice, where `describe` reads the
      // on-screen accessibility tree itself over the sim-server's axAudit
      // endpoint (see describe/platforms/ios/index.ts) and never resolves
      // ax-service — this is a backstop for any other direct ax-service
      // resolution.
      throw new FailureError(
        `${AX_SERVICE_NAMESPACE} is iOS-simulator-only. The physical device '${device.id}' is driven over CoreDevice — use describe, which reads its accessibility tree over CoreDevice directly, or screenshot.`,
        {
