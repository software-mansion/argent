...
function buildFullMarkdown(
  ...
  const lines: string[] = [
    ...
    `> On a grouped \`×N\` row, \`self\` is the total across the N instances while \`w/children\``,
    `> is the single most expensive instance's subtree (inclusive times cannot be added up`,
    `> across instances, because same-named instances are often nested inside one another).`,
    ``,
    `---`,
    `## Slow React Batches`,
    ``,
