...
function buildFullMarkdown(
  ...
  const lines: string[] = [
    ...
    `> \`w/children\` = self + the entire subtree it owns (inclusive), for ONE instance —`,
    `> on a grouped \`×N\` row it is the single most expensive of the N, while \`self\` is the`,
    `> total across all of them.`,
    `> Never sum \`w/children\`, across rows or across instances: a parent's inclusive time`,
    `> already contains its children's, and same-named instances are often nested inside one`,
    `> another. Sum \`self\`; read \`w/children\` to understand container cost.`,
    ``,
    `---`,
    `## Slow React Batches`,
    ``,
