...
### Drop CPU from profiler-analyze

CPU data is too noisy and hard to attribute to specific component issues without a full flamegraph. `profiler-cpu-summary` remains for raw CPU inspection, but `profiler-analyze` focuses entirely on React commit data from `__RN_DEVTOOLS_MCP_COMMITS__`.
...
### React Compiler detection moved to profiler-stop

Scan ALL commits in profiler-stop (before hot filtering) to set `anyCompilerOptimized`. Store in session. Pass via `sessionMeta.anyCompilerOptimized` into pipeline. Stage 1 (reduce) seeds `anyRuntimeCompilerDetected` from this flag so cold commits contribute to detection.
...
`Array<{offsetMs, label}>` where `offsetMs = Date.now() - profileStartWallMs` at action time. `profiler-start` records `profileStartWallMs = Date.now()` on the Node.js side. React commit timestamps are `performance.now()` from the device; commits are displayed with timestamps relative to the first commit (t=0 approximation).
...
Top 10 hot commits in inline response to stay token-efficient. Full markdown written to `rn-devtools-debug/profiler-report.md` for agent to re-query without re-running profiling. Agent can use the Read tool directly on the file.
