...
const zodSchema = z.object({
  ...
  rotation: z
    ...
    .optional()
    .describe(
      "Orientation override for the screenshot. Applied on Android and on local iOS simulators, and on Chromium after Page.captureScreenshot — where, like downscaling, it needs the optional `sharp` dependency. Apple TV, Vega, physical iPhones and remote iOS simulators accept it and capture unrotated. With no rotation passed, Android captures upright already — it follows the device's own rotation."
    ),
  scale: z
    .number()
    .min(0.01)
  ...
  downscaler: z
    ...
    .optional()
    .describe(
      "Downscaling algorithm when scale<1 on Chromium, where it goes through the same optional `sharp` dependency the downscale itself needs. Defaults to lanczos3 (highest quality). Mirrors sim-server's wire enum. Ignored on physical iPhones."
    ),
});
