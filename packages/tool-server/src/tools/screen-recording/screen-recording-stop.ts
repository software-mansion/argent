...
Returns { video, durationMs, wallClockMs?, trimmedMs?, panelSwitches?, warning? }; video is a downloadable artifact materialized to a local path. When static-frame trimming removed dead air, durationMs is the trimmed video length and wallClockMs/trimmedMs report the real duration and how much was cut. On a foldable simulator the recording follows the panel the device renders to; panelSwitches counts the folds it followed, the video keeps the size of the panel it started on, and warning says how many times (at the start, or on a check) the panel could not be resolved because neither the accessibility service nor CoreDevice answered, so the recording stayed where it was.
