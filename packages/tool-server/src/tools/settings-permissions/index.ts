...
Returns { action, permission, bundleId, applied, skipped?, unverified? }: \`applied\` lists the platform-level services/permissions actually changed; \`skipped\` (Android) lists mapped permissions that did not take effect, e.g. ones the manifest doesn't declare; \`unverified\` (Android) lists applied entries that could not be confirmed. Fails if nothing could be applied.`,
