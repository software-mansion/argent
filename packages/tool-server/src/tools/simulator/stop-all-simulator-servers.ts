...
Returns { stopped } - the URNs of the services that were actually live and got shut down; an ERROR node is disposed too but never appears there, so an empty \`stopped\` only means nothing was still running, and an ERROR node also does not count as ownership - a scope naming only devices whose sole service is a failed start still reports them in \`unmatched\`. { unmatched } lists supplied ids that own no service here, so a mistyped id - or a device NAME passed where an id was expected - does not read as a clean machine. It is NOT proof the id is wrong: a Vega device is driven through CLI/adb shell-outs, so one you only booted and drove with the remote registers no service and always lands here — as does a HarmonyOS device, driven entirely through \`hdc\` shell-outs (including one where a flow minted a dead ERROR node by running a tool the platform cannot serve), and a real device of any platform this session never started anything on. Present ONLY when \`devices\` was supplied AND at least one id matched nothing - absent on an unscoped call and when every id matched. Stopping the same device twice does not report it unmatched: its nodes remain as IDLE and still count as ownership; only an ERROR node does not. { left_running } lists live debugger sessions (and the network inspectors / React profiler sessions riding on them) whose id no device scope can name - re-call with that id to reap them. { aborted: true } means the caller cancelled the request part-way, so the rest of the machine was left untouched and neither of the other two fields was computed. Past the schema - which rejects an unknown key outright, so the \`udids\` slip is an error rather than a silent machine-wide sweep - the call always succeeds; reaping nothing is a result, not a failure.`,
    zodSchema,
    services: () => ({}),
    async execute(_services, params, ctx) {
      ...
      for (const [urn, entry] of snapshot.services) {
        ...
        // otherwise be reported unmatched by the next scoped call — EXCEPT an
        // ERROR node: a failed start (e.g. a flow run against HarmonyOS, whose
        // SimulatorServer blueprint refuses the platform) left behind a node
        // that never held anything, so counting it would hide the id from
        // `unmatched` and present `{ stopped: [] }` as a clean machine.
        if (matchedId !== undefined && entry.state !== ServiceState.ERROR) {
