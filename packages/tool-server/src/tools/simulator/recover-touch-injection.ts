...
Argent's normal recovery (stop-simulator-server, then re-boot) does NOT clear this; only restarting the host CoreSimulator daemon does. This tool disposes argent's services for EVERY local Apple simulator (the daemon restart is host-wide, so all of their handles go stale), runs \`xcrun simctl shutdown all\`, \`killall ${CORE_SIMULATOR_SERVICE}\`, then re-boots the target device (waiting for it) and restarts every other simulator that was booted, in the background (unless rebootAfter:false).
