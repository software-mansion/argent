...
Use when you need to scroll a list, dismiss a modal, drag an element, or navigate between pages. Not supported on Chromium — use gesture-scroll there instead. Returns { swiped: true, timestampMs }. On a physical iPhone, swipes register on the persistent WebDriverAgent queue. Fails if the device backend is not reachable.`,
  alwaysLoad: true,
  ...
  async execute(services, params) {
    ...
    if (isPhysicalIos(device)) {
      ...
      await physicalIos.swipe(params.fromX, params.fromY, params.toX, params.toY, duration);
