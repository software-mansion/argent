 ...
 * `flow-chromium-tree.ts`.
 *
 * The automation toolkit's page source is Vega's only tree source, so there is
 * no richer hierarchy to query; what the parsed tree lacks is the flow
 * contract: flat leaves under one root and `subtreeText` hoisting. The toolkit
 * puts an element's text on child `text` nodes (a nav `button` wrapping a
 * `text` leaf carrying "Home"), so without hoisting a `text`/`assert` check
 * against the button reads its own (empty) text.
 */

/**
 * Project a describe node for the shared flatten (see `flow-tree-flatten`).
 * Every parsed node becomes a leaf: the source parser already flattened bare
 * structural wrappers and dropped launcher/`<traits>` noise, so whatever
 * survived is addressable. Zero-area nodes stay too — `exists` deliberately
 ...
 * as its `identifier`), so letting those shield would disable hoisting.
 */
 ...
 * toolkit's empty tree + relaunch hint passes through so the runner's
 * blind-read guard still sees the hint.
 */
