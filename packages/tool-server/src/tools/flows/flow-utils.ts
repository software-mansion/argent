...
export function blockSteps(step: FlowStep): FlowStep[] | undefined {
  return step.kind === "when" || step.kind === "repeat" ? step.steps : undefined;
}
...
function parseWaitFields(
  ...
  if (raw === null || typeof raw !== "object") {
    badEntry(entry, `${label} needs a condition (${WAIT_CONDITIONS.join(", ")})`);
  }
  ...
  if (present.length !== 1) {
    badEntry(entry, `${label} needs exactly one condition key (${WAIT_CONDITIONS.join(", ")})`);
  }
 ...
 */
const BLOCK_DIRECTIVE_KEYS: readonly string[] = ["when", "repeat"];
