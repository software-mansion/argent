...
function partialMutationWarning(command: "flow-execute" | "run-sequence"): string {
  ...
  return (
    ...
    "Relaunching the app reproduces that prefix only while the prefix is the recorded launch " +
    "step alone; after any later step it lands on the start screen, which is not where the " +
    "prefix ends."
  );
}
...
A NORMAL return can also mean "not recorded": \`recorded\` is the discriminator - it is absent, and \`message\` says "step NOT recorded", whenever the call ran but its outcome must not become a step. That covers a nested \`flow-execute\` that failed, was cancelled, or returned a prerequisite notice instead of running, and a nested \`run-sequence\` that stopped on a failed nested step or was cancelled part-way. Read \`message\` before assuming the step landed. Whether anything ran is what \`message\` says, not something to infer from which case it was: it warns that the device may have moved whenever a nested step was reached, and stays silent when the refusal provably reached none: a prerequisite notice, a cancel that landed before the first dispatch, and a sequence that sent nothing - one rejected before its first step could be dispatched, and one whose completed steps were all \`await-ui-element\` waits, which poll the tree and act on nothing whether or not the condition held. On the warning, CHECK the device against the state your recorded prefix leaves it in before adding the next step. Relaunching the app to "reset" reproduces that state only while the prefix is the recorded \`launch\` step alone; after any later step it lands on the start screen instead, which makes the rest of the recording unreproducible.
