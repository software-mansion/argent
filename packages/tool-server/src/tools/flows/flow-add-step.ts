  ...
  command: z
    ...
    .describe(
      'MCP tool name (e.g. "gesture-tap", "screenshot", "launch-app") — a TOOL, not a flow directive. ' +
        ...
        "answered with guidance, and nothing runs or is recorded: most name the tool that records " +
        "the directive, while " +
        ...
        "flow-add-script, flow-start-recording, flow-finish-recording) is refused the same way, " +
...
const NESTED_RECORDER_TOOLS: Record<string, string> = {
  ...
  "flow-add-script":
    "`flow-add-script` runs the file and records the `script:` step itself, so it must be called " +
    "DIRECTLY, not through flow-add-step — nesting it would run the script AND write a " +
    "`tool: flow-add-script` step that re-runs it on every replay.",
  "flow-add-step":
...
export function directiveCommandHint(command: string): string | undefined {
  ...
  if (command === "script") {
    return (
      `"script" is a flow directive, not a tool. Call \`flow-add-script\` DIRECTLY — not through ` +
      `flow-add-step, which would run the script as a nested tool AND record a ` +
      `\`tool: flow-add-script\` step that runs it a second time on every replay.`
    );
  }
  if (command === "wait") {
    return (
...
Returns { message, toolResult, stepCount, recorded, savedTo } on success — \`message\` is \`Step added to "<name>" flow\` plus any warning about what was recorded (read it; a warning never means the step was skipped). Two calls SUCCEED while recording nothing, and omit \`recorded\` to say so: a \`command\` naming a recording tool, and one naming a flow-file directive rather than a tool. Both answer with what to do instead — usually the call to make (the tool that records that directive, or the recording tool called directly), but \`wait\`, \`long-press\`, \`scroll-to\`, \`snapshot\` and \`when\` have no recording tool, so those name no call and say what to record or add by hand in its place. Either way nothing runs at the device and the take is left untouched — read \`recorded\`, not the status, to know whether a step was appended. If it fails an error is returned and nothing is recorded; whether the tool call itself RAN depends on where it failed. The checks that precede the call — the recording must be open, \`args\` must parse, a recorded \`flow-execute\` must name a sibling flow rather than a \`flow_path\`, and \`args\` must satisfy the named tool's own schema — reject without running anything. Every later failure means the call ran and nothing it did was undone, so read the error before you retry: repeating such a call repeats the action.
