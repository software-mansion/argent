 ...
 * a `momentum: false` swipe (no fling); Chromium uses wheel events (already
 * momentum-free).
 */
 ...
 * `swipe` / `type` / `await` / `assert` / `scroll-to` / `pinch` / `rotate`)
 * plus `idle`, which takes no selector because stillness is a property of the
 * whole screen.
 */
