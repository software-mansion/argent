...
export async function startNativeProfilerIos(
  ...
  if (useMallocStackLogging) {
    ...
    // Pick the capture approach for this environment. A physical device always uses
    // the device/attach path: its app is not a host process, so the host-wide
    // --all-processes fallback (a simulator-only workaround for the Xcode 26.4-and-later
    // `xctrace --device` deadlock) cannot isolate it. For simulators, pick per Xcode
    // version — the original device/attach path (which attaches by PID, immune to
    // Xcode 26.5's display-name `--attach` matching), or the all-processes fallback
    // filtered to the app PID. See capture-strategy.
    strategy = isPhysicalDevice ? deviceStrategy : selectIosCaptureStrategy();
