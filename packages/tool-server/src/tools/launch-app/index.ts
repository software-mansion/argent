...
Returns { launched, bundleId, note? }. Fails if the app is not installed on the target device (iOS / Android). On a physical iPhone this also registers the app that describe, the wait tools and gestures act on; com.apple.springboard and com.apple.Spotlight register without launching. There, note warns when runner signing is not ready, before the next interaction tool would fail on it.
