...
- Android supports all ten, on emulators and real devices, via \`adb\` (\`cmd uimode night\`, \`font_scale\`, accessibility flags, \`svc wifi/data\`, \`cmd connectivity airplane-mode\`, \`location_mode\`, \`accelerometer_rotation\`). A setting the device's Android version doesn't implement fails with the device's own refusal instead of reporting a change it never made; \`location\` also needs Android 10 (API 29)+, where \`location_mode\` became the master switch, and is refused below it. On a wirelessly debugged device (a \`host:port\` serial) \`wifi\` off and \`airplane-mode\` on are refused — they would switch off the link adb reaches it over.
