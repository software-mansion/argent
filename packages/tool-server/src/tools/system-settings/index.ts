...
Platforms:
- iOS simulator supports the first five (display / accessibility): \`appearance\`, \`text-size\`, and \`increase-contrast\` via \`simctl ui\`; \`reduce-motion\` and \`invert-colors\` via the accessibility preferences domain. The simulator must be booted. A setting the runtime doesn't model fails with the runtime's own refusal and a hint to try a newer one, and the five Android-only settings are rejected on iOS. Apple TV simulators are rejected outright — neither mechanism reaches tvOS.
- Android supports all ten, on emulators and real devices, via \`adb\` (\`cmd uimode night\`, \`font_scale\`, accessibility flags, \`svc wifi/data\`, \`cmd connectivity airplane-mode\`, \`location_mode\`, \`accelerometer_rotation\`). A setting the device's Android version doesn't implement fails instead of reporting a change it never made; \`location\` also needs Android 10 (API 29)+, where \`location_mode\` became the master switch, and is refused below it. On a wirelessly debugged device (a \`host:port\` serial) \`wifi\` off and \`airplane-mode\` on are refused — they would switch off the link adb reaches it over.
