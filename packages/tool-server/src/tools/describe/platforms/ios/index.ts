...
export async function describeIos(
  ...
  let hint = resolverHint ?? degradedHint;

  // Both hints end in "boot-device with force=true", which is refused on a
  // provider's device. Keep the blind/standing split, redirect the remedy.
  if (
    isExternalId(device.id) &&
    (hint === DEGRADED_BLIND_HINT || hint === DEGRADED_STANDING_HINT)
  ) {
    hint = externalCaveat(
      device.id,
      hint === DEGRADED_BLIND_HINT ? EXTERNAL_BLIND_BODY : EXTERNAL_STANDING_BODY
    );
  }
