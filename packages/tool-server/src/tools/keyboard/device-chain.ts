 ...
 *     field fully SELECTED in between: a character arriving there replaces the
 *     whole value. The DEL consumes that selection, but a read-back and the
 *     rescue run it can call for still precede the text, and a character landing
 *     in that second window is measured as residue and backspaced away.
