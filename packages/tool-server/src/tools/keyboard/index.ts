...
// CDP session; the ios/android branches runtime-probe their TV kind (TV is a
// `runtimeKind`, not a `platform`, so a tvOS sim is "ios" and an Android TV
// "android" by id shape) and route a TV target to the focus-driven backend,
// otherwise to the simulator-server (see platforms/{ios,android,chromium,tv}.ts).
// No service is declared eagerly: distinguishing a TV target is async, and
// declaring simulator-server up front would also spawn it for a tvOS udid it
// can't drive.
export function createKeyboardTool(registry: Registry): ToolDefinition<Params, KeyboardResult> {
  return {
    id: "keyboard",
    description: `Type text or press special keys on the device (iOS simulator, Android emulator, Chromium app, or Apple TV / Android TV) using keyboard events.
Use when you need to enter text or trigger a named key such as enter, escape, or arrow keys.
Returns { typed: string, keys: number }. Fails if an unsupported key name is provided or the simulator-server / emulator backend / Chromium CDP / TV control daemons are not reachable for the given device.
...
Provide text, key, or both. Use instead of paste when paste is unreliable or unsupported by the focused field.`,
    ...
    capability,
    searchHint:
      "type text keyboard input named key enter escape arrow tv search field hid leanback",
    // No eager service: each branch resolves its backend lazily (TV control,
    // simulator-server, or CDP), since distinguishing a TV target is async and a
    // tvOS udid must never resolve simulator-server.
    services: () => ({}),
