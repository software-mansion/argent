  ...
  clear: z
    ...
    .describe(
      "Empty the focused text field before typing. Use this whenever a field may already hold a value — typing alone does NOT replace it: the old text survives and the new text goes in at the caret, which lands after the old value or splices into the middle of it depending on where focus left the caret. " +
        ...
        "What each backend does about a swallowed select-all: Android reads the field back after the chord and deletes whatever survived it, repairing the clear on a single-line field it can read; Chromium reads before and after and fails with an error when it observes the value survive, leaving the field for the caller to deal with; on iOS, which cannot read the field at all, a swallowed chord leaves the delete acting as a plain backspace AT THE CARET, so the field ends up one character shorter rather than unchanged and a combined `text` is appended at the end (where focusing jumped). " +
        "On Android the read-back needs the screen's view hierarchy and measures focused `EditText` nodes only; where the screen cannot be captured, where the budget is already spent, or where any focused field on it is one the hierarchy cannot read (a password box, a WebView input, a custom widget), no repair runs and the field is left one character shorter — with a combined `text` spliced in at the caret, which on Android is where the tap put it rather than the end. A field that reads back as exactly its own placeholder counts as empty, so a value that happens to equal the placeholder is left alone too. The repair itself deletes backwards from end-of-LINE, so a multi-line field keeps what sits below the caret. Levels older than `input keycombination` take that same end-of-LINE delete from the start, and there a field over 150 characters is refused rather than partly deleted; the same refusal can fire on a level that HAS the subcommand when the read-back measures more residue than backspaces are allowed to clear. On those older levels a length that cannot be read at all (a password field, or a screen the device would not capture) falls back to a fixed 158 backspaces from that same end-of-LINE start — more than every length that path accepts, so a single-line field keeps only its head past 158 characters, and a multi-line one keeps that head plus every line below the caret."
    ),
  delayMs: z
    .number()
...
Almost every rejection happens before the field is touched. An unsupported key name, un-typeable text, and a \`clear\` on a platform that cannot do it are all decided before anything is sent, on every backend; the Android over-length refusal is decided from a READ of the field, so a support probe and a screen dump go to the device first — neither of which changes it. Those 400s leave the field exactly as it was, neither emptied nor half-typed. ONE 400 does not: on an Android level with \`input keycombination\`, the same over-length refusal can also come from the read-back taken AFTER the select-all and the delete have gone out, and the message says so when it does. Two things are NOT rolled back, and they leave different residue: that 400, where the field keeps whatever the select-all and the delete did to it, and a transport failure partway through, where the characters already sent stay in it. On a retry after either, read the field's actual contents — do not assume it is unchanged.
