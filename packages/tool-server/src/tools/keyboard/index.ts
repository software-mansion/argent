...
Returns { typed: string, keys: number, cleared?: boolean, note?: string }. \`note\` is an advisory present only when there is one: on Android a \`clear\` that could not take the atomic accessibility path says there why, which weaker path ran, and what that path cannot promise about the field — so no note means the clear was the verified one. Fails if text and key are both given in one call (rejected before anything is typed), if an unsupported key name is provided, if \`clear\` is used on a platform that cannot do it, or if the device's input backend is not reachable.
