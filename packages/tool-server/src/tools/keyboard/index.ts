...
Returns { typed: string, keys: number }. On a physical iOS device the result also carries reactivated: true when the target app was backgrounded and the runner had to re-front it to deliver the input (the foreground screen changed as a side effect). Fails if text and key are both given in one call (rejected before anything is typed), if an unsupported key name is provided, or if the device's input backend is not reachable.
...
- key: presses a single named key (enter, escape, backspace, tab, arrow-up/down/left/right, f1-f12). NOT supported on TV targets; move focus with \`tv-remote\` instead. On physical iOS devices only \`enter\` and \`backspace\` are supported; type text, or use \`gesture-tap\` to press on-screen keys.
