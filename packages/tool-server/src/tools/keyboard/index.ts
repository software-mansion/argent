...
Fails if text and key are both given in one call (rejected before anything is typed), if an unsupported key name is provided, if the device's input backend is not reachable, or if the caller cancels — a cancellation seen before the first keystroke types nothing, and the Android phone/tablet, iOS simulator and Chromium backends also stop between keys once one arrives mid-call (on a TV target the daemon types the whole string regardless). A read-back that cannot run never fails the call: the text is typed either way.
