...
const zodSchema = z.object({
  ...
  key: z
    ...
    .optional()
    .describe(
      "Named key to press: enter, escape, backspace, tab, space, arrow-up, arrow-down, arrow-left, arrow-right, f1–f12. On HarmonyOS only enter (return), backspace (delete), space, arrow-left and arrow-right exist — any other key fails with 400 KEYBOARD_KEY_UNSUPPORTED. Cannot be combined with `text` in one call — one call per action; to type and then press a key, put two `keyboard` steps in one `run-sequence`. Not supported on TV targets — move focus with `tv-remote` (up/down/left/right) instead."
    ),
  delayMs: z
    .number()
    .optional()
