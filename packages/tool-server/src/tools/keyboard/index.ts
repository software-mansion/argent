  ...
  clear: z
    ...
    .describe(
      "Empty the focused text field before typing. Use this whenever a field may already hold a value — typing alone does NOT replace it: the old text survives and the new text goes in at the caret, which lands after the old value or splices into the middle of it depending on where focus left the caret. " +
        ...
        "What each backend can leave behind, since only Chromium reads the field back: on iOS and Android a widget that swallows the select-all leaves the following delete acting as a plain backspace AT THE CARET, so the field ends up one character shorter rather than unchanged — and a combined `text` then goes in at that same caret, appended on iOS (where focusing jumps to the end) but spliced into the middle on Android (where it lands where the tap did). " +
