...
A failure is not rolled back: an unsupported key name or un-typeable text is rejected before anything is sent, but if the transport fails partway the text already dispatched stays in the field — so on a retry, check the field's actual contents rather than assuming it is unchanged.
