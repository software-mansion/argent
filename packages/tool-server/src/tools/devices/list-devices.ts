...
Use it to see which targets are already running, and to pick an id when there is a choice to make: booting something, naming one of several booted devices, or filling boot-device's 'udid' or the debugger and profiler tools' 'device_id'. The interaction tools take 'udid' as an optional argument and resolve the single booted device themselves, so a session with one device needs no listing first. Ids are 'udid' for iOS entries, 'serial' for Android/Vega entries, 'id' for Chromium.
Returns { devices, avds } where each device carries a 'platform' discriminator ('ios', 'ios-remote', 'android', 'chromium', or 'vega'); an 'ios-remote' entry is a simulator on a remote host, and the interaction tools reach it only when you pass its 'udid'. 'avds' lists Android AVDs bootable via boot-device. A Vega VVD is listed under 'devices' whether running or stopped (state 'running'/'stopped'); start a stopped one with boot-device using its 'vvdImage'.
