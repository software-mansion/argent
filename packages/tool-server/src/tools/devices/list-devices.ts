...
TV targets are tagged with runtimeKind 'tv' (Apple TV simulators on iOS, Android TV / leanback devices on Android) — these are focus-driven, not touch-driven: use \`describe\` to read focus, \`button\` for remote presses (up/down/left/right/select/menu/home/playpause), \`keyboard\` to type, and \`tv-set-focus\` to jump focus by label, rather than the coordinate/gesture tools.
