...
Booted/ready devices are listed first. Platforms whose CLI is unavailable are silently omitted — an empty result usually means xcode-select, Android platform-tools, or the Vega SDK is not installed.`,
  ...
  async execute(_services, params) {
    ...
    // The physical-device scan is opt-in (default off, not part of the session-start
    // path) and self-bounds via its own 15s devicectl timeout, so it stays outside
    // the fan-out backstop and returns null when not requested.
    const [ios, iosRemote, android, avds, chromium, vega, physical] = await Promise.all([
    ...
    > = [...iosSimTagged, ...iosPhysTagged, ...iosRemote, ...androidDeduped, ...chromium, ...vega];
