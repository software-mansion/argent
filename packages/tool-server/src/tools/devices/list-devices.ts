...
function readinessRank(
  d: IosDevice | IosPhysicalDevice | IosRemoteDevice | AndroidDevice | ChromiumDevice | VegaDevice
): number {
...
Chromium apps are discovered by probing CDP debugging ports (9222, whatever ARGENT_CHROMIUM_PORTS=<comma-separated-ports> lists, and the ports boot-device itself opened). They must already be running with --remote-debugging-port=<port> — use boot-device with electronAppPath to launch one. A missing Chromium entry does not mean the app exited: a probe needs a drivable page, so an app that is up with no window is dropped exactly as an exited one is — and for a port boot-device opened, that probe failure also drops it from the set this tool probes, so the entry may not come back when the app does. Keep the id boot-device returned. The other two sources are rebuilt every call, so only a boot-device port can be lost this way.
Booted/ready devices are listed first. Platforms whose CLI is unavailable are silently omitted — an empty result usually means xcode-select, Android platform-tools, or the Vega SDK is not installed.`,
  ...
  async execute(_services, _params) {
    ...
    const devices: Array<
      IosDevice | IosPhysicalDevice | IosRemoteDevice | AndroidDevice | ChromiumDevice | VegaDevice
    > = [...iosTagged, ...iosPhysicalTagged, ...iosRemote, ...androidDeduped, ...chromium, ...vega];
