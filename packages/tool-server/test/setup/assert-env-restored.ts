...
// stubbed binary is found instead of the real one. Restoring is the half that
// is easy to omit and impossible to notice: under the shipped `isolate: true`
// each file gets its own fork, so a suite that leaves a variable naming a
// deleted directory takes the evidence with it when the fork exits.
//
