/**
 * Regression: when ios-profiler-stop has populated `exportedFiles.cpu` with
...
import { iosInstrumentsAnalyzeTool } from "../src/tools/profiler/ios-profiler/ios-profiler-analyze";
import type { IosProfilerSessionApi } from "../src/blueprints/ios-profiler-session";

describe("ios-profiler-analyze: missing trace file", () => {
  it("surfaces an export warning that names the missing CPU file when its path is set but the file is absent", async () => {
    let dir: string | null = null;
    try {
      dir = await mkdtemp(join(tmpdir(), "ios-profiler-missing-"));
      ...
      // Simulate what ios-profiler-stop hands to ios-profiler-analyze:
      // exportedFiles has a non-null CPU path, but the file does not exist.
      const session: IosProfilerSessionApi = {
      ...
      const result = await iosInstrumentsAnalyzeTool.execute(
        { session },
